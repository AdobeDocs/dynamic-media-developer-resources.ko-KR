---
title: 재생/일시 중지 단추
description: 재생/일시 중지 버튼은 사용자가 비디오 컨텐츠를 선택할 때 비디오 플레이어가 비디오 컨텐츠를 재생하거나 일시 중지하도록 합니다.
...
재생/일시 중지 버튼은 사용자가 비디오 컨텐츠를 선택할 때 비디오 플레이어에서 비디오 컨텐츠를 재생하거나 일시 중지하도록 합니다.
...
>
>`replay='true'` 이 속성은 비디오가 끝에 도달하고 버튼을 선택하면 처음부터 재생이 다시 시작될 때 설정됩니다.
...
32x32픽셀이며 컨트롤 막대의 위쪽 및 왼쪽 가장자리에서 6픽셀을 배치하는 재생/일시 중지 단추를 설정하려면 마지막으로, 을 선택하거나 선택하지 않았을 때 4개의 서로 다른 단추 상태 각각에 대해 다른 이미지를 표시합니다.
