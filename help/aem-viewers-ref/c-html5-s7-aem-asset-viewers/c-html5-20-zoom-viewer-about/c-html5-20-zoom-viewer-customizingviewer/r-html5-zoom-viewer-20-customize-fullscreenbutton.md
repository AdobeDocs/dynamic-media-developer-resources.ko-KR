---
title: 전체 화면 단추
description: 사용자가 뷰어를 선택하면 뷰어가 전체 화면 모드로 전환되거나 종료됩니다. 뷰어가 팝업 모드에서 작동하고 시스템이 기본 전체 화면을 지원하지 않는 경우에는 이 단추가 표시되지 않습니다. CSS를 사용하여 이 버튼의 크기를 조정하고, 스킨을 지정하고, 위치를 지정할 수 있습니다.
