...
IS-Viewer와 함께 제공되는 프로덕션 준비 HTML 페이지나 포함된 모드에서 문서화된 API를 사용하여 대상 웹 페이지에 통합되는 eCatalog Search Viewer를 팝업 모드에서 사용할 수 있습니다.
...
팝업 작업 모드에서는 기본 HTML 페이지를 사용하는 것이 좋습니다. 이 경우 라고 합니다 [!DNL eCatalogSearchViewer.html] 및 는 [!DNL html5/] 표준 IS-Viewers 배포의 하위 폴더:
...
1. 웹 페이지에 뷰어 JavaScript 파일 추가.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 다음을 포함해야 합니다 [!DNL eCatalogSearchViewer.js]. 다음 [!DNL eCatalogSearchViewer.js] 파일은 [!DNL html5/js/] 표준 IS-Viewers 배포의 하위 폴더:
...
1. 뷰어 크기 설정
   ...
   뷰어에 대한 정적 크기를 `.s7ecatalogsearchviewer` 절대 단위의 최상위 CSS 클래스 또는 `stagesize` 수정자.
