...
* `initialframe` 전자 카탈로그의 모바일 장치에서 세로 모드에 영향을 주지 않습니다.
...
* Chrome 브라우저에서 실행 중인 마우스 및 터치 입력 지원을 추가했습니다. Microsoft Surface 장치가 이 범주에 속하지만 Microsoft Edge는 5.5 뷰어에서 터치 입력과 함께 작동하지 않습니다.

## 5.5.1에 대한 버그 수정 {#section-c64001afa2aa460180141bc8dcd507ec}

* eCatalog 뷰어에서 오른쪽에서 왼쪽으로 읽는 방향 지원.
* `tip=0,-1,0` 가 범위를 벗어나는 오류를 일으켰습니다.
...
* `InitalFrame` 여러 페이지 스프레드에서 맨 왼쪽 페이지에만 링크를 추가할 수 있습니다. 세로 모드의 모바일 장치에 영향을 줍니다.
