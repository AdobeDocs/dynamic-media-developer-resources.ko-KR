...
*`pixelType`* 지정된 색상 공간 `icc=` 이 아닌 경우 효과 출력 색상 공간 변환 에 해당하는 기본 색상 프로필 *`pixelType`* 이 적용됩니다. 색상 관리를 사용하지 않도록 설정하면 자동 전환이 적용됩니다. *`pixelType`* 이 지정되면 이  `icc=` 무시됩니다. 이 값은 출력 픽셀 유형을 결정합니다.
