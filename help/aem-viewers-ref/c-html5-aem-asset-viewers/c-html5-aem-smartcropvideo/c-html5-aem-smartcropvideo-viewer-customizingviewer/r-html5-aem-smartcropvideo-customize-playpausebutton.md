---
title: 재생/일시 중지 단추
description: 재생/일시 중지 버튼을 클릭하면 스마트 자르기 비디오 플레이어에서 비디오 콘텐츠를 재생하거나 일시 중지합니다.
...
>
>속성 선택기 `replay='true'`은(는) 비디오가 종료되면 설정되며, 버튼을 선택하면 처음부터 재생이 다시 시작됩니다.
...
32 x 32픽셀로 재생/일시 중지 단추를 설정합니다. 컨트롤 막대의 위쪽 및 왼쪽 가장자리에서 6픽셀 떨어진 위치에 배치합니다. 마지막으로, 선택하거나 선택하지 않았을 때 4개의 서로 다른 단추 상태 각각에 대해 다른 이미지를 표시합니다.
