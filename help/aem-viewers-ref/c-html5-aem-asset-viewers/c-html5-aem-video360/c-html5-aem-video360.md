...
HTML5 비디오360 뷰어는 런타임에 뷰어가 다운로드한 기본 JavaScript 파일 및 도우미 파일 세트(단일 JavaScript에 이 뷰어에서 사용하는 모든 HTML5 Viewer SDK 구성 요소, 에셋, CSS가 포함됨)를 나타냅니다.
...
1. 뷰어 크기 설정
   ...
   CSS에서 크기를 HTML 페이지에서 바로 지정할 수도 있고, 나중에 Adobe Experience Manager Assets의 뷰어 사전 설정 레코드에 할당되거나 `style` 명령을 사용하여 명시적으로 전달되는 사용자 지정 뷰어 CSS 파일에서 크기를 지정할 수도 있습니다.
