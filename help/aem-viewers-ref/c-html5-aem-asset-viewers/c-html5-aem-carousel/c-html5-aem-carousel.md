---
description: Carousel Viewer는 클릭 가능한 핫스팟 또는 영역이 있는 확대/축소할 수 없는 배너 이미지의 Carousel을 표시하는 뷰어입니다. 이 뷰어의 목적은 사용자가 배너 이미지 위에 있는 핫스팟 또는 지역을 선택하고 고객 웹 사이트의 Quickview 또는 제품 세부 사항 페이지로 리디렉션할 수 있는 "쇼퍼블 슬라이드" 경험을 구현하는 것입니다. 데스크탑과 모바일 디바이스에서 작동하도록 설계되었습니다.
seo-description: Carousel Viewer는 클릭 가능한 핫스팟 또는 영역이 있는 확대/축소할 수 없는 배너 이미지의 Carousel을 표시하는 뷰어입니다. 이 뷰어의 목적은 사용자가 배너 이미지 위에 있는 핫스팟 또는 지역을 선택하고 고객 웹 사이트의 Quickview 또는 제품 세부 사항 페이지로 리디렉션할 수 있는 "쇼퍼블 슬라이드" 경험을 구현하는 것입니다. 데스크탑과 모바일 디바이스에서 작동하도록 설계되었습니다.
seo-title: 회전식
...
Carousel Viewer는 클릭 가능한 핫스팟 또는 영역이 있는 확대/축소할 수 없는 배너 이미지의 Carousel을 표시하는 뷰어입니다. 이 뷰어의 목적은 사용자가 배너 이미지 위에 있는 핫스팟 또는 지역을 선택하고 고객 웹 사이트의 Quickview 또는 제품 세부 사항 페이지로 리디렉션할 수 있는 &quot;쇼퍼블 슬라이드&quot; 경험을 구현하는 것입니다. 데스크탑과 모바일 디바이스에서 작동하도록 설계되었습니다.
...
Carousel Viewer는 런타임에 뷰어가 다운로드한 기본 JavaScript 파일 및 도우미 파일 집합(단일 JavaScript는 이 특정 뷰어에서 사용하는 모든 뷰어 SDK 구성 요소, 자산, CSS)을 나타냅니다.

Carousel Viewer는 IS Viewers와 함께 제공되는 프로덕션 준비 HTML 페이지를 사용하거나 문서화된 API를 사용하여 대상 웹 페이지에 통합되는 임베디드 모드에서 팝업 모드에서 모두 사용할 수 있습니다.

구성 및 스키닝은 이 도움말에 설명된 다른 뷰어와 유사합니다. 모든 스키닝은 맞춤형 CSS를 통해 이루어집니다.
...
회전판 세트 탐색은 기본 보기 위로 가로로 스와이프하거나 데스크톱 장치에서 사용할 수 있는 두 개의 화살표 단추를 사용하여 수행됩니다. 세트 표시기 점들은 세트 내의 현재 위치를 보여줍니다.

뷰어는 배너 이미지 위에 있는 핫스팟 또는 영역을 렌더링하여 제품의 대화형 영역을 나타낼 수 있습니다.

핫스팟 또는 영역을 클릭하거나 탭하면 작성자 시간 동안 핫스팟과 연관된 작업이 트리거됩니다. 작업은 웹 사이트의 다른 페이지로 리디렉션되거나 제품 정보를 웹 페이지 로직으로 다시 전달할 수 있으며, 이렇게 하면 관련 제품 컨텐츠가 포함된 빠른 보기가 트리거될 수 있습니다.

뷰어는 키보드로 완벽하게 액세스할 수 있습니다.

키보드 [접근성 및 탐색을](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861)참조하십시오.

## 회전식 뷰어 포함 {#section-6bb5d3c502544ad18a58eafe12a13435}
...
팝업 작업 모드에서는 즉시 사용 가능한 HTML 페이지를 사용하는 것이 좋습니다. 이 경우 이 매개 변수를 `CarouselViewer.html` 호출하고 표준 IS-뷰어 배포의 `html5/` 하위 폴더 내에 있습니다.
...
내장 모드에서는 뷰어가 기존 웹 페이지에 추가됩니다. 이 웹 페이지에는 이미 뷰어와 관련이 없는 일부 고객 콘텐츠가 있을 수 있습니다. 뷰어는 일반적으로 웹 페이지의 부동산의 일부만을 차지합니다.
...
반응형 디자인 임베드 모드에서 뷰어는 웹 페이지의 컨테이너 크기 조절 방식에 따라 다르게 동작합니다 `DIV`. 웹 페이지가 컨테이너의 너비만 설정하고 높이를 제한을 두지 `DIV`않으면 뷰어는 사용된 자산의 종횡비에 따라 높이를 자동으로 선택합니다. 이 기능을 사용하면 측면에 패딩 없이 에셋이 보기에 완벽하게 맞춰집니다. 이 사용 사례는 Bootstrap, Foundation 등과 같은 반응형 웹 디자인 레이아웃 프레임워크를 사용하는 웹 페이지에 가장 일반적으로 사용됩니다.

그렇지 않으면 웹 페이지가 뷰어의 컨테이너에 대한 너비와 높이를 모두 설정하는 `DIV`경우 뷰어는 해당 영역만 채웁니다. 또한 웹 페이지 레이아웃이 제공하는 크기를 따릅니다. 예를 들어 뷰어를 모달 오버레이에 포함시켜 오버레이의 크기가 웹 브라우저 창 크기에 따라 조정되는 것이 좋습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일을 추가합니다.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 포함해야 [!DNL CarouselViewer.js]합니다. 이 [!DNL CarouselViewer.js] 파일은 표준 IS-뷰어 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어 크기 설정
   ...
   뷰어의 정적 크기를 절대 단위로 `.s7carouselviewer` 최상위 CSS 클래스에 대해 선언하거나 `stagesize` 수정자를 사용하여 설정할 수 있습니다.

   HTML 페이지 또는 사용자 정의 뷰어 CSS 파일에서 바로 크기를 조정할 수 있습니다. 이 파일은 나중에 AEM Assets의 뷰어 사전 설정 레코드에 할당되거나 `style` 명령을 사용하여 명시적으로 전달됩니다.

   CSS를 [사용하여](../../c-html5-aem-asset-viewers/c-html5-aem-carousel/c-html5-aem-carousel-customizingviewer/c-html5-aem-carousel-customizingviewer.md#concept-73a8546acdb444a387c49969ceca57d0) 뷰어의 스타일 지정에 대한 자세한 내용은 회전판 뷰어 사용자 지정을 참조하십시오.
   ...
   다음과 같이 명령 참조 섹션에 설명된 대로 뷰어 초기화 코드를 `stagesize` `params` 컬렉션이나 API 호출로 수정자를 명시적으로 전달할 수 있습니다.
...
1. 뷰어를 만들고 초기화합니다.
   ...
   다음은 필요한 최소 구성 옵션을 생성자에게 전달하고 `init()` 메서드를 호출하는 뷰어 인스턴스를 만드는 예입니다. 이 예제에서는 뷰어 `carouselViewer` 인스턴스가 있다고 가정합니다.자리 표시자의 `s7viewer` 이름입니다. `DIV`;이미지 제공 `https://adobedemo62-h.assetsadobe.com/is/image` URL이며 `/content/dam/dm-public-facing-live-demo-page/04_shoppable_carousel/05_shoppable_banner` 자산입니다.
   ...
   다음 코드는 고정된 크기의 회전판 뷰어를 포함하는 간단한 웹 페이지의 전체 예입니다.
...
반응형 디자인 임베드에서는 일반적으로 웹 페이지에 뷰어의 컨테이너 런타임 크기를 지정하는 유연한 레이아웃이 `DIV`있습니다. 다음 예에서는 웹 페이지에서 뷰어의 컨테이너를 웹 브라우저 창 크기의 40% `DIV` 를 사용할 수 있다고 가정합니다. 그리고, 그것의 높이는 제한 없이 남겨집니다. 웹 페이지 HTML 코드는 다음과 같습니다.
...
위의 모든 단계는 고정 크기 임베딩과 동일합니다. 기존 `DIV` 컨테이너에 컨테이너를 `"holder"` 추가합니다 `DIV`. 다음 코드는 전체 예입니다. 브라우저 크기를 조정할 때 뷰어 크기가 어떻게 변경되고 뷰어 종횡비가 자산과 어떻게 일치하는지 확인합니다.
