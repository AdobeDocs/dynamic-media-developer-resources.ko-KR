---
description: 클릭유도문안 패널은 비디오가 종료되고 특정 비디오와 연결된 모든 대화형 견본을 표시할 때 나타납니다.
seo-description: 클릭유도문안 패널은 비디오가 종료되고 특정 비디오와 연결된 모든 대화형 견본을 표시할 때 나타납니다.
seo-title: 조치 수행 질문
...
클릭유도문안 패널은 비디오가 종료되고 특정 비디오와 연결된 모든 대화형 견본을 표시할 때 나타납니다.

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

패널은 비디오 제목을 표시하는 머리글 영역, 오른쪽 위 모서리에 있는 재생 단추 및 스크롤 가능 격자로 표시되는 실제 대화형 견본으로 구성됩니다. callToActionRecap 구성 특성을 사용하여 패널을 [비활성화할 수](../../../c-html5-aem-asset-viewers/c-html5-aem-int-video/r-html5-aem-int-video-config-attrib/r-html5-aem-int-video-config-attrib-calltoactionrecap.md#reference-3720b68800684ddabf523e9d81644ce6) 있습니다.

클릭유도문안 패널은 항상 사용 가능한 전체 뷰어 영역을 사용합니다.

<!--<a id="section_3A619BE925C04AFA87A6B7846C5C7E2B"></a>-->

다음 CSS 클래스 선택기는 작업 패널에 대한 호출에서 배경색의 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction
```

## 클릭유도문안 배경색의 CSS 속성 {#css-properties-of-the-background-color-of-the-call-to-action-panel}

<table id="table_C48C56E696304C9BAFEE71BA9EA9A174"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> background-color </span> </p> </td> 
   <td colname="col2"> <p> 클릭 유도 문안 패널의 배경색입니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
어두운 회색 배경이 있는 작업 패널에 대한 호출을 설정하려면
...
다음 CSS 클래스 선택기는 작업 패널에 대한 호출에서 헤더의 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction .s7header
```

## 액션 패널 헤더의 CSS 속성 {#css-properties-of-the-call-to-action-panel-header}
...
70픽셀의 높이와 어두운 회색의 배경이 있고 아래쪽을 따라 약간 밝은 회색의 2픽셀 테두리를 설정하는 방법은 다음과 같습니다.
...
다음 CSS 클래스 선택기는 작업 패널에 대한 호출에서 머리글 제목의 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction .s7header .s7title
```

## 클릭유도문안 내 머리글 제목의 CSS 속성:  {#css-properties-of-the-header-title-in-the-call-to-action-panel}

<table id="table_A5E36A5C4C664346B6DAE9A02B36C3D2"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> color </span> </p> </td> 
   <td colname="col2"> <p> 배너 안의 텍스트 색상입니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> [재생] 단추의 공간을 허용하는 오른쪽 패딩. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
70픽셀 선 높이, 25픽셀 글꼴 크기, 흰색 및 왼쪽 정렬로 비디오 제목을 설정하려면 다음을 수행하십시오.
...
다음 CSS 클래스 선택기는 클릭 유도 문안 패널에서 닫기 단추 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction .s7closebutton
```

## 클릭유도문안 패널에서 닫기 단추의 CSS 속성: {#css-properties-of-the-close-button-in-the-call-to-action-panel}

<table id="table_CB0BCBE70DB447BC8D31034A96308924"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 최상위 </span> </p> </td> 
   <td colname="col2"> <p>패딩을 포함하여 머리글 위쪽에서 위치를 지정합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 오른쪽 </span> </p> </td> 
   <td colname="col2"> <p>패딩을 포함하여 헤더 오른쪽에서 위치를 지정합니다. </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>이 버튼은 `state` 속성 선택기를 지원합니다. 이 선택기는 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다.

## 예 {#example-3}

28 x 28픽셀인 재생 단추를 설정하려면위치의 위쪽 및 오른쪽 가장자리에서 20픽셀서로 다른 네 가지 단추 상태에 대해 다른 이미지를 표시합니다.구성 요소의 스프라이트 이미지에서 아트웍을 가져옵니다.
...
다음 CSS 클래스 선택기는 클릭 유도 액션 패널에서 축소판 격자 보기의 모양을 제어합니다.
...
다음 CSS 클래스 선택기는 클릭 유도 문안 패널에서 축소판 셀의 모양을 제어합니다.
...
<table id="table_9CEBEF6FC7024F02840A581AEEF612B4"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 각 축소판의 가로 및 세로 여백의 크기입니다. </p> <p>실제 가로 축소판 간격은 <span class="codeph"> .s7thumbcell에 대해 설정된 왼쪽 및 오른쪽 여백의 </span>합계입니다. 세로 간격에도 동일한 규칙이 적용됩니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
24픽셀의 수평 간격 및 18픽셀의 수직 간격을 설정하려면
...
다음 CSS 클래스 선택기는 클릭 유도 액션 패널에서 축소판의 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction .s7thumb
```

## 클릭유도문안 패널에 있는 축소판의 CSS 속성: {#css-properties-of-the-thumbnail-in-the-call-to-action-panel}
...
>[!NOTE]
>
>축소판은 `state` 속성 선택기를 지원합니다. 이 선택기는 다른 축소판 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히, `state="selected"` 현재 선택된 이미지의 축소판에 해당합니다.나머지 축소판에 `state="default"` 해당합니다.마우스를 가리키면 `state="over"` 사용됩니다.
...
다음 CSS 클래스 선택기는 작업 패널에 대한 호출에서 축소판 레이블의 모양을 제어합니다.
...
<table id="table_E2C9F21EBD9140FD9D20A4BBAD117E2F"> 
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> text-align </span> </p> </td> 
   <td colname="col2"> <p>레이블의 가로 맞춤입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> font-family </span> </p> </td> 
...
흰색 색상을 사용하는 레이블을 설정하려면 가운데 정렬된 15픽셀을 사용하고 Arial 글꼴을 사용합니다.
...
보기에 세로로 맞출 수 있는 축소판이 많을 경우 축소판은 오른쪽에 세로 스크롤 막대를 렌더링합니다. 기본적으로 [동작] 패널에서는 축소판 및 스크롤 단추 없이 작은 세로 막대가 렌더링됩니다. 그러나 뷰어 CSS를 변경하여 막대를 사용자 정의할 수 있습니다.

다음 CSS 클래스 선택기는 작업 패널에 대한 호출에서 스크롤 막대 영역의 모양을 제어합니다.

```
.s7interactivevideoviewer .s7calltoaction .s7thumbnailgridview .s7scrollbar
```

## 클릭 유도 문안 패널에서 스크롤 막대 영역의 CSS 속성: {#css-properties-of-the-scroll-bar-area-in-the-call-to-action-panel}
...
<table id="table_6D3A4A68BFDB44259A6E2E632B9195F3"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>축소판 영역의 아래쪽에서 세로 스크롤 막대 오프셋을 사용합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 오른쪽 </span> </p> </td> 
   <td colname="col2"> <p> 축소판 영역의 오른쪽 가장자리에서 수평 스크롤 막대 오프셋을 적용할 수 있습니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
축소판 영역의 위쪽, 오른쪽 또는 아래쪽의 여백이 없고 폭이 22픽셀인 스크롤 막대를 설정하려면:
...
다음 CSS 클래스 선택기는 클릭 유도 문안 패널에서 스크롤 막대 트랙의 모양을 제어합니다.
...
가로 22픽셀이고 회색이 있는 스크롤 막대 트랙을 설정하려면:
...
스크롤 막대 축소판 표시는 스크롤 트랙 영역 내에서 세로로 이동합니다. 세로 위치는 구성 요소 로직에 의해 완전히 제어됩니다.그러나 축소판 높이는 컨텐츠 양에 따라 동적으로 변경되지 않습니다.

다음 CSS 클래스 선택기는 축소판 높이와 다른 종횡의 모양을 제어합니다.
...
<table id="table_1F39948FC3924FA4B7F851B65B2D860B"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>트랙 아래쪽 세로 패딩. </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>Thumb는 `state` 속성 선택기를 지원합니다. 이 선택기는 다음과 같은 다른 thumb 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. `"up"`, `"down"`, `"over"`및 `"disabled"`를 참조하십시오.

## 예 {#example-10}

6 x 167픽셀인 스크롤 막대 축소판을 설정하려면 세 픽셀 모서리와 회색 색상을 사용합니다.
...
>[!NOTE]
>
>이러한 버튼은 `state` 속성 선택기를 지원합니다. 이 선택기는 다음과 같은 다른 축소판 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. `"up"`, `"down"`, `"over"`및 `"disabled"`를 참조하십시오.
...
스크롤 단추의 크기를 0으로 설정하고 크기를 숨겨 다음 단추를 비활성화합니다.
