...
플라이아웃 뷰어는 이미지 뷰어입니다. 사용자가 활성화한 플라이아웃 보기에 축소된 버전이 표시된 정적 이미지가 표시됩니다. 이 뷰어는 이미지 세트와 연동하며 견본을 사용하여 탐색합니다. 데스크탑과 모바일 디바이스에서 작동하도록 설계되었습니다.
...
웹 페이지마다 뷰어 비헤이비어에 대한 요구 사항이 다릅니다. 웹 페이지에는 정적 페이지 레이아웃이 있거나, 다른 장치 또는 다른 브라우저 창 크기에 대해 다르게 표시되는 반응형 디자인을 사용할 수 있습니다. 이러한 요구 사항을 수용하기 위해 뷰어는 두 가지 기본 작업 모드를 지원합니다.고정 크기 임베딩 및 반응형 디자인 임베딩.
...
플라이아웃 뷰어에서 반응형 디자인 임베드 모드를 사용할 때는 `imagereload` 매개 변수를 사용하여 기본 보기 이미지에 대한 명시적 중단점을 지정해야 합니다. 가장 좋은 방법은 웹 페이지 CSS에서 지정한 뷰어 폭 중단점과 중단점을 일치시키는 것입니다.
...
1. 뷰어를 만들고 초기화합니다.
   ...
   다음은 뷰어 인스턴스를 만들고 필요한 최소 구성 옵션을 생성자에게 전달하고 `init()` 메서드를 호출하는 예제입니다. 이 예제에서는 `flyoutViewer`이(가) 뷰어 인스턴스라고 가정합니다.`s7viewer`은 자리 표시자 `DIV`;의 이름입니다.`http://s7d1.scene7.com/is/image/`은(는) 이미지 제공 URL입니다.및 `Scene7SharedAssets/ImageSet-Views-Sample`은(는) 자산입니다.
   ...
   다음 코드는 고정 크기의 플라이아웃 뷰어를 포함하는 간단한 웹 페이지의 완전한 예입니다.
...
* 절대 단위를 사용하여 고정 뷰어 크기를 설정하는 대신 다음과 같이 뷰어 폭과 높이를 100%로 설정하는 CSS를 사용합니다.
