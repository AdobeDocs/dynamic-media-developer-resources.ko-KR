...
설정 *`chroma`* 일반적인 JPEG 인코더에서 사용되는 색도 다운샘플링을 비활성화하는 플래그입니다. 이 설정은 에지가 밝기가 아닌 색조의 변화에 의해 정의될 때 이미지에서 에지의 인지된 선명도를 증가시킬 수 있다. 이 플래그를 설정하면 파일 크기가 약간 증가할 수 있습니다. 텍스트가 약간 흐리게 보이는 경우 이 설정을 실험해 보십시오.

## 속성 {#section-897b61c786dd4230a2c5807f2f40e722}

요청의 어느 위치에서나 발생할 수 있습니다.

출력 이미지 형식이 JPEG 압축을 지원하지 않는 경우 무시됩니다. 다음에 대한 설명을 참조하십시오. `fmt=` JPEG 압축을 지원하는 출력 이미지 형식 목록입니다.
