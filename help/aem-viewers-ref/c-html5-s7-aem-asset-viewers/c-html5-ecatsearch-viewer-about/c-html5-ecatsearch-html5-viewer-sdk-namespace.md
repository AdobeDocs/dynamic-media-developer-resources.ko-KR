...
예를 들어 `StatusEvent.NOTF_VIEW_READY` 이벤트를 수신하고 뷰어를 Dynamic Media Classic에서 제공하는 경우, 정규화된 이벤트 유형은 `s7classic.s7sdk.event.StatusEvent.NOTF_VIEW_READY`이고 이벤트 리스너 코드는 다음과 비슷합니다.
