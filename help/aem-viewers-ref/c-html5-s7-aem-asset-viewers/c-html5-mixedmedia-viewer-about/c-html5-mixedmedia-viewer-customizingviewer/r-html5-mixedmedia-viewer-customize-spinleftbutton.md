...
이 버튼을 선택하면 이미지가 기본 보기에서 왼쪽으로 회전합니다. 이 버튼은 화면 부동산을 저장하기 위해 휴대폰에는 표시되지 않습니다. 또한 다차원 회전 집합을 사용하면 단추가 표시되지 않습니다. CSS를 사용하여 버튼의 크기를 조정하고, 스킨을 지정하고, 위치를 지정할 수 있습니다.
...
예 - 28 x 28픽셀이며 내부 컨테이너의 왼쪽 가장자리에 있는 회전 왼쪽 단추를 설정합니다. 그리고 마지막으로, 에는 네 가지 서로 다른 버튼 상태에 대해 각각 다른 이미지가 표시됩니다.
