...
[!DNL Adobe Scene7 Publishing System]에서 [!DNL Adobe Experience Manager] 솔루션의 [!DNL Adobe Experience Cloud] 6.10 부분에 대한 새로운 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보입니다.
...
<table id="table_69203DAA18F94A0F927BBA00634EBF65"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol">(으)로 시작하는 </span> </p> </td>
...
<table id="table_E78C924E34894CD7BE080C0D71500784"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>자동 회전 애니메이션을 활성화하거나 비활성화합니다. 기본값은 <span class="uicontrol">에서 </span>입니다. <span class="uicontrol">에서 </span>(으)로 설정된 경우 회전 기간을 지정할 수 있습니다. 회전 기간은 1회의 전체 회전 시간(초)으로, 기본값은 <span class="codeph"> 5 </span>입니다. 회전 방향을 오른쪽 또는 왼쪽으로 설정합니다. 기본값은 <span class="uicontrol"> 오른쪽 </span>입니다. </p> </td>
  </tr>
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="uicontrol">에서 </span> 고해상도 미리 로드 <span class="uicontrol">을(를) </span>(으)로 설정하면 구성된 프레임 범위가 고품질로 로드됩니다. 이 설정을 사용하면 대역폭 사용과 페이지 로드 시간이 크게 늘어날 수 있습니다. </p> </td>
  </tr>
 </tbody>
</table>
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 왼쪽 레일에서 <span class="uicontrol"> 플라이아웃/인라인 확대/축소 </span>을(를) 클릭하여 사용 가능한 다음 옵션에 액세스합니다. </p> <p> 
     <ul id="ul_081A473EFAAC47DD8CF5260492A53C67"> 
      ...
      <li id="li_C240206B2E3F4919935C05A253022ED9"> <span class="uicontrol"> 확대/축소 이미지 미리 로드 </span>(기본값: <span class="codeph"> 해제 </span>) - <span class="uicontrol">(1)에서 </span>(으)로 설정하여 확대/축소된 이미지의 미리 로드를 활성화하고 필요에 따라 확대/축소 이미지를 점진적으로 로드하려면 <span class="uicontrol">(0)에서 </span>(으)로 설정합니다. <p>이 옵션을 활성화하면 사용자가 확대/축소 작업을 실행하지 않아도 확대/축소된 이미지가 전부 로드되기 때문에 대역폭 사용률이 크게 증가합니다. </p> </li> 
      <li id="li_3B9FDD1C82E94E1AB77055B76947F110"> <span class="uicontrol"> 데스크톱용 </span> 팁 텍스트 - 데스크톱 시스템용 뷰어에 표시되는 선택적 팁에 대한 텍스트를 지정할 수 있습니다. </li> 
...
* API `<metadataArray>`에 대한 응답으로 [!DNL .vtt] 파일에 대해 `getAssets`이(가) 비어 있습니다.
...
* SDK 구성 요소에서 가시성:visible 스타일 사용을 제거했습니다.
...
* 고정 - ZoomView - CSS의 IMG에 대해 최대 너비:100%가 정의된 경우 IE에서 축소 및 패닝됩니다.
* 상태 이벤트가 추가되었습니다.
...
* 태그 유형 메타데이터 필드를 만들고 업데이트하기 위한 구성 사용자 인터페이스를 구현했습니다. **[!UICONTROL 매개 변수에서]**&#x200B;메타데이터 필드 형식`fieldType`을 지원합니다. `SingleFixedTag` 및 `BooleanTag`만 포함됩니다.
...
* 유형 **[!UICONTROL 의 사용자 정의 필드(]**&#x200B;설정&#x200B;**>**&#x200B;메타데이터&#x200B;**>**&#x200B;사용자 정의 필드`Boolean`)와 값 `true`을(를) 추가한 다음 저장하는 동안 오류가 발생했습니다.
