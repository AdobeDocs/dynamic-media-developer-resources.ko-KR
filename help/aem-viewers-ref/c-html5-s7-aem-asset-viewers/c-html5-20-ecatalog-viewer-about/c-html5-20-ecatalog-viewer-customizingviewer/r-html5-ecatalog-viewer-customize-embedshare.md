...
예 - 패딩, 24 x 14픽셀 아이콘 및 굵은 16포인트 제목이 있는 대화 상자 헤더를 설정하려면 다음을 수행하십시오. 또한 28x28픽셀 닫기 버튼이 있으며 맨 위에는 2픽셀, 대화 상자 컨테이너 오른쪽에는 2픽셀이 있습니다.
...
예 - 64 x 34 취소 단추, 82 x 34 모두 선택 단추 및 텍스트 색상과 배경색이 각 단추 상태에 따라 다른 대화 상자 바닥글을 설정하려면 다음을 수행합니다.
...
대화 상자 레이블은 현지화할 수 있습니다. 다음을 참조하십시오 [사용자 인터페이스 요소의 현지화](../../../c-html5-s7-aem-asset-viewers/c-html5-20-ecatalog-viewer-about/c-html5-20-ecatalog-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 추가 정보.
...
예제 - 사용할 포함 코드를 설정하려면 `break-word` 단어 줄바꿈:
...
포함 크기 레이블 및 드롭다운이 대화 상자 하단에 있고 다음 CSS 클래스 선택기로 제어되는 컨테이너에 삽입됩니다.
...
>[!NOTE]
>
>콤보 상자는 `expanded` 가능한 값이 있는 속성 선택기 `true` 및 `false`. 다음 `true` 콤보 상자에 사전 정의된 포함 크기 중 하나가 표시되면 값이 사용되므로 사용 가능한 모든 너비를 사용해야 합니다. 다음 `false` 콤보 상자에서 사용자 지정 크기 옵션을 선택한 경우 값이 사용되므로 사용자 지정 너비 및 높이 입력 필드에 공백을 허용하도록 축소해야 합니다.
...
예 - 28 x 45픽셀이고 위쪽과 아래쪽에 10픽셀 여백이 있으며 각 상태에 대해 서로 다른 아트워크가 있는 스크롤 막대 썸을 설정하려면 다음을 수행합니다.
...
버튼 도구 팁은 현지화할 수 있습니다. 다음을 참조하십시오 [사용자 인터페이스 요소의 현지화](../../../c-html5-s7-aem-asset-viewers/c-html5-20-ecatalog-viewer-about/c-html5-20-ecatalog-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 추가 정보.
