...
* **새로운 설정이 기본 제공 HTML5 회전 집합 뷰어에 추가됨** - **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 뷰어]** > **[!UICONTROL 뷰어 사전 설정]** > `Universal_HTML5_SpinSet_dark` 또는 `Universal_HTML5_SpinSet_light`에서 각 뷰어의 각 [!DNL Configure Viewer] 페이지에 다음과 같이 추가되었습니다.
...
* **새로운 설정이 기본 제공 HTML5 eCatalog 뷰어에 추가됨** - **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 뷰어 사전 설정]** > `Universal_HTML5_eCatalog` 또는 `Universal_HTML5_eCatalog_Adv`에서 각 뷰어의 각 [!DNL Configure Viewer] 페이지에 다음과 같은 사항이 추가되었습니다.
...
* **기본 제공 HTML5 혼합 미디어 집합 뷰어에 새 설정이 추가되었습니다** - **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 뷰어 사전 설정]** > `Universal_HTML5_MixedMedia_dark` 또는 `Universal_HTML5_MixedMedia_light`에서 각 뷰어의 각 [!DNL Configure Viewer] 페이지에 다음과 같은 설정이 추가되었습니다.
...
* IPS Classic Publish 유효성 검사 - 의 카탈로그 서버 재동기화 옵션이 제거되었습니다.
...
컴퓨터가 스테이징 서버에 액세스하도록 설정한 후 웹 사이트를 확인하여 업그레이드를 테스트할 수 있습니다. 기본 제공 HTML 5 뷰어를 사용하는 경우 Adobe의 표준 스테이징 서버 및 s7is5-preview-staging.scene7.com에 대해 테스트하는 것이 좋습니다.
...
* Publish 설정에 Scene7 공유 Assets 회사의 색상 프로필이 표시되지 않았습니다.
* FTP 화면에서 업로드: FTP 폴더는 정렬되지 않았지만 출력은 정렬된 폴더 목록입니다.
* Media Portal 설정에서 그룹 편집에 대한 &quot;자산 액세스 권한&quot; 탭의 설명이 변경되었습니다.
* 보안 테스트 이미지 서비스 Publish 설정을 사용하는 동안 형식이 잘못된 `clientAddressFilter`이(가) 카탈로그 서버에서 예외를 발생시켰습니다.
* URL에 연속된 여러 공백이 있는 경우 Chrome 브라우저에서 PDF을 위한 URL 복사가 실패합니다.
...
* **Flash 뷰어 사용 종료 알림** *2017년 1월 31일부로 Adobe Scene7(현재 Dynamic Media Classic)에서 공식적으로 Flash 뷰어 플랫폼에 대한 지원을 종료했습니다.*
