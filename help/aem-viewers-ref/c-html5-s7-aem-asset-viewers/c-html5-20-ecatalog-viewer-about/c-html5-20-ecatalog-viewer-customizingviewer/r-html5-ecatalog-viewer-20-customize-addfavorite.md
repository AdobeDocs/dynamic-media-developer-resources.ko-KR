...
>[!NOTE]
>
>이 단추는 `state` 및 `selected` 특성 선택기를 모두 지원합니다. 이 선택기는 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히 `selected='true'`은(는) 사용자가 선택하거나 탭하여 새 즐겨찾기 아이콘을 추가할 수 있는 상태입니다. `selected='false'` 특성은 사용자가 페이지를 확대/축소, 패닝 및 교환할 수 있는 일반 작업 모드에 해당합니다.
...
예 - 28 x 28픽셀 크기의 즐겨찾기 추가 단추를 설정합니다. 그리고 이 옵션을 선택하면 선택하거나 선택하지 않을 때 4개의 서로 다른 단추 상태 각각에 대해 다른 이미지가 표시됩니다.
