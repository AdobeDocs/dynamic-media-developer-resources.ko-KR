---
description: 닫힌 캡션 표시를 켜거나 끕니다. 캡션 매개 변수가 지정되지 않은 경우 표시되지 않습니다. CSS를 사용하여 이 버튼을 포함하는 컨트롤 막대를 기준으로 크기를 조정하고, 스킨을 지정하고, 위치를 지정할 수 있습니다.
seo-description: 닫힌 캡션 표시를 켜거나 끕니다. 캡션 매개 변수가 지정되지 않은 경우 표시되지 않습니다. CSS를 사용하여 이 버튼을 포함하는 컨트롤 막대를 기준으로 크기를 조정하고, 스킨을 지정하고, 위치를 지정할 수 있습니다.
seo-title: 캡션 단추
...
닫힌 캡션 표시를 켜거나 끕니다. 캡션 매개 변수가 지정되지 않은 경우 표시되지 않습니다. CSS를 사용하여 이 버튼을 포함하는 컨트롤 막대를 기준으로 크기를 조정하고, 스킨을 지정하고, 위치를 지정할 수 있습니다.
...
>[!NOTE]
>
>이 단추는 `state` 속성 선택기 및 선택한 속성 선택기를 지원합니다. 이 선택기는 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히, 캡션을 볼 수 `selected='true'` 있는 상태와 캡션을 숨길 때 `selected='false'` 사용되는 상태에 해당합니다.
...
예 - 28 x 28픽셀인 자막 단추를 설정하고 컨트롤 막대의 오른쪽 가장자리에서 4픽셀, 오른쪽 가장자리에서 68픽셀을 배치하여 선택 또는 선택 안 함에 따라 각각 다른 4개의 단추 상태에 대해 다른 이미지를 표시합니다.
