...
예 - 반투명 회색 배경을 사용하여 가로 100픽셀인 즐겨찾기 보기를 설정하려면 다음을 수행합니다.
...
>[!NOTE]
>
>축소판 그림 은 `state` 속성 선택기. 다른 스킨을 다른 축소판 상태에 적용하는 데 사용할 수 있습니다. 특히, `state="selected"` 사용자가 최근에 선택한 축소판에 해당합니다. 속성 `state="default"` 축소판의 나머지 부분에 해당합니다. 그리고, 속성 `state="over"` 마우스로 가리키면 사용됩니다.
...
예 - 14픽셀 Helvetica® 글꼴로 레이블을 설정하려면 다음을 수행합니다.
