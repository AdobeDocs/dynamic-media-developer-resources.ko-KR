...
RTF 여백 명령 `\margl`및 `\margr``\margt`이 있으면 `\margb` 무시됩니다 `textFlowPath=` . 경로 정의를 지정하지 않으면 `textFlowPath=` 무시됩니다.

## 속성 {#section-b68dc887c6534ce8982cad740b3aeaa4}

텍스트 레이어 특성( `textPs=` 전용). 다른 레이어에서 무시됩니다. 에 대해 지정된 `layer=0` 경우 적용됩니다 `layer=comp`.
