...
선택적 추가 `appName` 및 `appVersion` 매개 변수 `authHeader` 호출 애플리케이션을 추적하려면 에 로깅을 추가했습니다. `ipsApiService.log`.
...
사용되지 않음 `saveMetadataField` 새로운 것을 위한 작전 `createMetadataField` 및 `updateMetadataField` 작업.
...
또한 다음 필드는 두 필드에 모두 포함되지 않습니다 `getJobLogs` 및 `getJobLogDetails`. 이전 버전에서는 `getJobLogDetails`.

* `endDate` (작업이 완료된 경우)
* `fileDuplicateCount` (이전에는 항상 `0` with `getJobLogs`)
* `fileUpdateCount` (이전에는 항상 `0` with `getJobLogs` 및 `fileSuccessCount`; 이제 별도의 필드로 분할됩니다.
...
<table id="table_DCCE936B797A448598C30E3B344525A5"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>.xls로 끝나는 파일에 대한 Microsoft® Excel 문서. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_D4E5ACCB2D144D05A5FA0129AA5F9344"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col4"> <p> 
     <ul id="ul_14D0A696DF4E408DA50E102057EB7AC7"> 
      ...
      <li id="li_3F56CEABAB3E43EAB157C83583A2F58D"> <p>지정된 해상도 및 색상 공간에서 EPS 및 PostScript 파일을 이미지에 렌더링합니다. </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p> <span class="codeph"> alpha </span> </p> <p>선택 사항입니다. </p> </td> 
   <td colname="col3"> <p> <span class="codeph"> &lt;부울&gt; </span> </p> </td> 
   <td colname="col4"> <p>파일을 이미지로 래스터화할 때 적용됩니다. 원본 파일이 로고를 오버레이하기 위해 이 방식으로 정의된 경우 투명 배경을 만듭니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p> </p> </td> 
   <td colname="col4"> <p>파일을 이미지로 래스터화할 때 적용됩니다. 오버레이 로고를 만드는 방법으로 원본 파일이 정의된 경우 투명 배경을 만듭니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p> <span class="codeph"> &lt;부울&gt; </span> </p> </td> 
   <td colname="col4"> <p>PDF의 단어를 나중에 검색 서버에 제공할 수 있도록 DB로 추출할지 여부를 정의합니다(기본값은 false). </p> </td> 
  </tr> 
 </tbody> 
</table>
