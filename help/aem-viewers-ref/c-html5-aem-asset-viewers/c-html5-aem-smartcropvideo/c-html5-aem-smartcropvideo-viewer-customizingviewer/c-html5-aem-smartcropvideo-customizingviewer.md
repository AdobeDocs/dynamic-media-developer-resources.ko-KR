...
사용자 지정 CSS를 만들 때는 뷰어가 해당 컨테이너 DOM 요소에 `.s7smartcropvideoviewer` 클래스를 할당해야 합니다. `style=` 명령을 사용하여 전달된 외부 CSS 파일을 사용하는 경우 CSS 규칙의 하위 항목 선택기에서 `.s7smartcropvideoviewer` 클래스를 상위 클래스로 사용합니다. 웹 페이지에 스타일을 포함하는 경우 다음과 같이 이 선택기에 컨테이너 DOM 요소의 ID도 부여합니다.
...
* CSS 내의 외부 에셋에 대한 모든 경로는 뷰어 HTML 페이지의 위치가 아닌 CSS 위치에 대해 확인됩니다. 기본 CSS를 다른 위치에 복사할 때에는 이 규칙을 기억하십시오. 기본 에셋을 복사하거나 사용자 지정 CSS 내의 경로를 업데이트합니다.
