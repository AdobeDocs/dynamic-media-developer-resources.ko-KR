...
기본적으로 모달 대화 상자는 데스크톱 시스템의 화면에 중앙에 표시되며 터치 장치의 전체 웹 페이지 영역을 가져옵니다. 모든 경우 대화 상자의 위치와 크기는 구성 요소에 의해 관리됩니다. 대화 상자는 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
예 - 패딩이 있는 대화 상자 헤더를 설정하려면 24 x 17픽셀 아이콘, 굵은 16pt 제목 및 28 x 28픽셀 닫기 단추 위에서 2픽셀, 대화 상자 컨테이너 오른쪽에서 2픽셀을 지정합니다.
...
각 양식 입력 필드는 컨테이너에 둘러싸여 입력 필드 주위에 사용자 정의 테두리를 적용할 수 있습니다. 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
대화 상자 본문의 오른쪽 가장자리까지 왼쪽의 레이블에서 퍼진 대부분의 입력 필드(&quot;보낸 사람&quot; 필드 및 &quot;메시지&quot; 필드 포함)는 다음과 같이 제어됩니다.
...
&quot;받는 사람&quot; 입력 필드는 오른쪽의 &quot;이메일 제거&quot; 단추에 공간을 할당하므로 더 좁습니다. 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
예 - 모든 입력 필드 주위의 패딩이 9픽셀인 1픽셀 회색 테두리를 갖도록 양식을 설정하려면유효성 검사에 실패한 필드에 대해 동일한 테두리를 빨간색 색상으로 표시하려면 250픽셀 너비의 &quot;끝&quot; 입력 필드와 300픽셀 너비의 입력 필드를 사용합니다.
...
&quot;다른 이메일 주소 추가&quot; 단추를 사용하면 전자 메일 양식에 수신자를 두 명 이상 추가할 수 있습니다. 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
예 - &quot;다른 이메일 주소 추가&quot; 버튼을 25픽셀로 설정하려면, 오른쪽 정렬에 12포인트 굵은 글꼴과 각 상태에 대해 다른 텍스트 색상 및 이미지를 사용합니다.
...
공유 중인 컨텐츠는 대화 상자 본문의 하단에 표시되며 축소판, 제목, 원본 URL 및 설명이 포함되어 있습니다. 이 컨테이너는 다음 항목으로 제어됩니다.
...
오류 메시지를 지역화할 수 있습니다. 자세한 [내용은 사용자 인터페이스 요소의](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 현지화를 참조하십시오.
...
예 - 가로 28픽셀의 스크롤 막대를 설정하려면 스크롤 패널의 위쪽, 오른쪽 및 아래쪽에서 8픽셀 여백을 설정합니다.
