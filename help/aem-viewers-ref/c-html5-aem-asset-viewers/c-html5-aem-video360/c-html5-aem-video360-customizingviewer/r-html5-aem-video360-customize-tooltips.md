...
>[!NOTE]
>
>포함 웹 페이지 내에서 도구 설명 스타일을 사용자 지정하는 경우 모든 속성에 `!IMPORTANT` 규칙이 포함되어야 합니다. 도구 설명이 뷰어의 CSS 파일 내에서 사용자 지정된 경우에는 필요하지 않습니다.

예 - 회색 테두리로 테두리가 3픽셀 모퉁이 반경, 검정 배경 및 흰색 텍스트가 Arial로 설정된 도구 설명을 설정하려면 11픽셀입니다.
