...
대화형 이미지 뷰어는 런타임 시 뷰어가 다운로드한 기본 JavaScript 파일 및 도우미 파일 집합(단일 JavaScript는 이 특정 뷰어에서 사용하는 모든 뷰어 SDK 구성 요소, 자산, CSS)을 나타냅니다.

대화형 이미지 뷰어는 내장 모드에서만 사용할 수 있으며, 여기서 문서화된 API를 사용하여 대상 웹 페이지에 통합됩니다.
...
## 인터랙티브한 이미지 뷰어와 상호 작용 {#section-642e66ca38cd4032992840ec6c0b0cd2}

비디오 이미지 뷰어에서 지원하는 상호 작용은 데스크탑 시스템에서 핫스팟 활성화입니다. 이 활성화는 클릭 및 터치 디바이스에서 한 번의 터치로 이루어집니다.
...
대화형 이미지 뷰어는 호스팅 페이지에 포함되도록 디자인되었습니다. 이러한 웹 페이지에는 정적 레이아웃이 있거나, &quot;응답 속도&quot;가 있을 수 있으며, 다른 장치 또는 서로 다른 브라우저 창 크기에 대해 다르게 표시될 수 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일을 추가합니다.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 포함해야 [!DNL InterativeImage.js]합니다. 이 [!DNL InteractiveImage.js] 파일은 표준 IS-뷰어 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어 크기 설정
   ...
   뷰어의 정적 크기를 절대 단위로 `.s7interactiveimage` 최상위 CSS 클래스에 대해 선언하거나 `stagesize` 수정자를 사용하여 설정할 수 있습니다.
   ...
   CSS [로](../../c-html5-aem-asset-viewers/c-html5-aem-interactive-images/c-html5-aem-interactive-image-customizingviewer/c-html5-aem-interactive-image-customizingviewer.md#concept-73a8546acdb444a387c49969ceca57d0) 뷰어의 스타일 지정에 대한 자세한 내용은 비디오를 참조하십시오.
...
1. 뷰어를 만들고 초기화합니다.
   ...
   다음은 필요한 최소 구성 옵션을 생성자에게 전달하고 `init()` 메서드를 호출하는 뷰어 인스턴스를 만드는 예입니다. 이 예제에서는 뷰어 `interactiveImage` 인스턴스가 있다고 가정합니다.자리 표시자의 `s7viewer` 이름입니다. `DIV`;이미지 제공 `http://aodmarketingna.assetsadobe.com/is/image` URL이며 `/content/dam/mac/aodmarketingna/shoppable-banner/shoppable-banner.` 자산입니다.
   ...
   다음 코드는 고정된 크기의 비디오 이미지 뷰어를 포함하는 간단한 웹 페이지의 전체 예입니다.
