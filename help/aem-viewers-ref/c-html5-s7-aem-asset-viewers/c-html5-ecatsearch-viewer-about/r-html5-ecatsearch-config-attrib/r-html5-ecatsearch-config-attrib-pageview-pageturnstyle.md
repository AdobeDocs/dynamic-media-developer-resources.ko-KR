...
[!DNL `[PageView.|<containerId>_pageView.]pageturnstyle= *`구분선 너비`*, *`구분선 색상`*, *`구분선 불투명도`*, *`테두리 설정/해제`*, *`borderColor`*, *`채우기 색상`*`]

다음과 같은 경우 구성 요소 모양 제어 [!DNL `PageView.frametransition`] 이(가) (으)로 설정됨 [!DNL `turn`] 또는 종료 [!DNL `auto`] 데스크탑 시스템에서.
