...
컨테이너 요소가 아직 웹 페이지 레이아웃의 일부가 아닌 경우(예: `display:none` 스타일을 사용하여 숨길 수 있음) 뷰어는 초기화 프로세스를 일시 중단합니다. 웹 페이지가 컨테이너 요소를 레이아웃으로 다시 가져오는 순간까지 그렇게 됩니다. 이 프로세스가 발생하면 뷰어 로드가 자동으로 다시 시작됩니다.
