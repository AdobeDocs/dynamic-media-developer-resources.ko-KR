...
[!DNL Adobe Scene7 Publishing System]에서 Adobe Experience Manager 솔루션의 [!DNL Adobe Experience Cloud] 6.9 부분에 대한 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보입니다.
...
* **응답형 지원 향상** - 각 중단점에 대해 서로 다른 이미지 사전 설정 또는 수정자를 정의하여 응답형 이미징 경험의 품질을 더욱 최적화합니다. [응답형 정적 이미지 라이브러리 사용](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html?lang=ko)을 참조하세요.

## 비디오 {#section-69101b28504543a7ae8a9346d381f913}

* 이제 모든 종횡비와 함께 작동하며 모바일, 태블릿 및 데스크톱에 전송할 비디오를 만드는 새로운 &quot;적응형 비디오&quot; 인코딩 사전 설정입니다. 이 새 사전 설정으로 인코딩된 업로드된 소스 비디오는 고정된 높이로 설정되며, 비디오 종횡비를 유지하기 위해 너비가 자동으로 조절됩니다. [비디오 파일 인코딩을 위한 비디오 사전 설정](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html?lang=ko#setup)을 참조하십시오.
* 인코딩으로 탄력적인 확장성과 성능을 개선했습니다.
* 이제 기본 제공 `Universal_HTML5_MixedMedia_dark` 및 `Universal_HTML5_MixedMedia_light` 뷰어에서 비디오 캡션을 사용할 수 있습니다. [비디오에 캡션 추가](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html?lang=ko#video)를 참조하십시오.
...
* 렌더링 서버 요청에 대한 보안 테스트 - 이제 핵심 이미지 서비스 자산 및 비디오 자산에 대한 보안 테스트가 비네팅도 포함하도록 확장되었습니다. 공개하기 전에 [자산 테스트](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html?lang=ko#upload-publish)를 참조하십시오.
* 무효화 CDN 캐시 기능 모니터링. CDN 무효화 템플릿을 선택하고([일반 설정](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html?lang=ko#general-settings) 참조) **[!UICONTROL CDN 무효화]** 요청이 수행된 후 표시기에 캐시를 지우는 데 걸리는 시간이 예상되어 표시됩니다.
...
[Scene7 뷰어 참조 안내서](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/homeviewers.html?lang=ko)도 참조하세요.

* Internet Explorer 11에 대한 지원이 업데이트되었습니다.
* Adobe Analytics 추적에 `Track requests` 대신 `Page requests`을(를) 사용하도록 뷰어를 변경했습니다.
