...
의 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보 [!DNL Adobe Scene7 Publishing System] 6.8 부분이 포함된 Adobe Experience Manager 솔루션 [!DNL Adobe Experience Cloud].
...
* 추가됨 `SetIndicator` HTML5 Zoom Viewer 및 HTML5 Mixed Media Viewer를 사용하여 모바일 장치에서 견본을 시각적으로 나타냅니다.
* 추가됨 `getComponent` HTML5 뷰어 구성 요소에 대한 외부 액세스를 허용하기 위한 API입니다.
...
* `initComplete` 뷰어 다음에 함수를 트리거하기 위한 HTML5 뷰어 처리기입니다.
...
* 숨기려면 확대/축소 보기 `IconEffect` 구성 요소(확대/축소가 불가능할 때)
...
* 기본 역할을 제거했습니다.
* 이제 ftp(서버 디렉토리 업로드)를 통해 Scene7 Publishing System에 컨텐츠를 업로드하고 URL을 업로드하는 기능 외에도 HTTP POST을 사용하여 컨텐츠를 업로드할 수 있습니다. 다음을 참조하십시오. [이미지 프로덕션 시스템 API](/help/aem-ips-api/c-overview.md) HTTP POST 사용에 대한 자세한 내용.
...
* 매개 변수 `contenturl` 은(는) Publish 서버 HTML5 뷰어 미리보기 URL에서 가져온 것입니다.
...
* &quot;Space After&quot;는 작동하지 않습니다. `sa`, 현재 `margb`.
...
다음을 참조하십시오 [https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html)
...
* 수정됨: ThumbnailGridView가 전송되지 않음 `AssetEvent.ASSET_CHANGED`.

**VideoPlayer**

* 에 대한 지원이 추가됨 `initialBitRate` 데스크탑에서 초기 재생에 사용되는 비트 전송률을 선택하는 수정자입니다.
