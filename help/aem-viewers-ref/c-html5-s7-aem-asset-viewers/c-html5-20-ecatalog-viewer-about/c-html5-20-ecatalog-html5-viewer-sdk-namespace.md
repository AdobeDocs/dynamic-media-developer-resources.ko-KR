...
뷰어에서 SDK 구성 요소를 로드하고 초기화하는 데 사용되는 네임스페이스는 뷰어가 작동하는 환경에 따라 다릅니다. 뷰어가 AEM(Adobe Experience Manager)에서 실행 중인 경우 뷰어는 SDK 구성 요소를 `s7viewers.s7sdk` 네임스페이스로 로드합니다. SPS에서 제공되는 뷰어는 SDK를 `s7classic.s7sdk`에 로드합니다.
...
예를 들어 `StatusEvent.NOTF_VIEW_READY` 이벤트를 수신하고 뷰어를 SPS에서 제공하는 경우 정규화된 이벤트 유형은 `s7classic.s7sdk.event.StatusEvent.NOTF_VIEW_READY`이고 이벤트 리스너 코드는 다음과 비슷합니다.
