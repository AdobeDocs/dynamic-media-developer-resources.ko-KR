...
사용자 지정 CSS를 만들 때 뷰어가 해당 컨테이너 DOM 요소에 `.s7videoviewer` 클래스를 할당한다는 것을 기억하십시오. `style=` 명령으로 전달되는 외부 CSS 파일을 사용하는 경우 CSS 규칙에 대해 하위 선택기에서 `.s7videoviewer` 클래스를 상위 클래스로 사용합니다. 웹 페이지에 스타일을 포함하는 경우 이 선택기를 다음과 같이 컨테이너 DOM 요소의 ID로 추가로 분류합니다.
...
다음 샘플 CSS는 재생/일시 정지 단추 크기를 마우스 입력이 있는 시스템에서는 28 x 28픽셀, 터치 장치에서는 56 x 56픽셀로 설정합니다. 또한 뷰어 크기가 정말 작으면 버튼을 완전히 숨깁니다.
...
CSS 마커를 사용하는 것은 장치 화면 크기뿐만 아니라 실제 뷰어 크기를 타깃팅할 수 있으므로 응답형 디자인 페이지 레이아웃을 만드는 가장 유연한 방법입니다.
...
* CSS 내의 외부 자산에 대한 모든 경로는 뷰어 HTML 페이지의 위치가 아닌 CSS 위치에 대해 확인됩니다. 기본 CSS를 다른 위치에 복사할 때에는 이 규칙을 고려해야 합니다. 기본 자산을 복사하거나 사용자 지정 CSS 내에서 경로를 업데이트합니다.
