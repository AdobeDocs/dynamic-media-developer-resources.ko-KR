...
>[!NOTE]
>
>포함 웹 페이지 내에서 도구 설명 스타일을 사용자 지정하는 경우 모든 속성을 포함해야 합니다 `!IMPORTANT` 규칙. 뷰어의 CSS 파일에서 도구 설명이 사용자 지정된 경우에는 이 규칙이 필요하지 않습니다.

예 - 세 픽셀 모퉁이 반경, 검정 배경 및 Arial®로 작성된 흰색 텍스트가 있는 회색 테두리를 설정하는 도구 설명을 설정하려면 11픽셀 크기를 지정합니다.
