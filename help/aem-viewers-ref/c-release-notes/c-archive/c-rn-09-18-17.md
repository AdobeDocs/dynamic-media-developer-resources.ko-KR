...
* 사용자가 `frametransition=turn`인 경우 마우스를 사용하여 페이지를 켤 수 없습니다.
* `ImageMapEffect.mapTips=1` 마우스 클릭으로 터치 사용 장치에서 스크립트 오류가 발생했습니다.
...
* `playback` 한정자의 값 `native`은(는) 더 이상 사용되지 않습니다. 대신 새 값 `progressive`을(를) 사용하십시오.
...
* 포스터 이미지가 처음 방문자의 ® Edge 브라우저에 표시되지 않을 수 있습니다.
