---
description: eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, eCatalog를 사용하면 추가 사용자 인터페이스 요소나 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 모든 페이지를 확대하여 보다 자세히 볼 수 있습니다.
keywords: responsive
seo-description: eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, eCatalog를 사용하면 추가 사용자 인터페이스 요소나 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 모든 페이지를 확대하여 보다 자세히 볼 수 있습니다.
seo-title: eCatalog
...
eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, eCatalog를 사용하면 추가 사용자 인터페이스 요소나 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 모든 페이지를 확대하여 보다 자세히 볼 수 있습니다.

>[!NOTE]
>
>IR(Image Rendering) 또는 UGC(User-Generated Content)를 사용하는 이미지는 이 뷰어에서 지원되지 않습니다.
...
eCatalog 뷰어는 뷰어가 런타임 시 다운로드한 기본 JavaScript 파일 및 도우미 파일 집합(단일 JavaScript는 이 특정 뷰어에서 사용하는 모든 뷰어 SDK 구성 요소, 자산, CSS)을 나타냅니다

IS-Viewers와 함께 제공되거나 포함된 모드에서 프로덕션 준비 HTML 페이지를 사용하여 팝업 모드에서 eCatalog Viewer를 사용할 수 있습니다. 이 HTML은 문서화된 API를 사용하여 대상 웹 페이지에 통합됩니다.
...
eCatalog 뷰어는 다른 모바일 응용 프로그램에서 일반적으로 사용되는 다음 터치 제스처를 지원합니다.
...
이 뷰어는 키보드 액세스 가능성 및 탐색 [에 설명된 대로 완전히 키보드 액세스 가능합니다](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861).

## eCatalog 뷰어를 사용한 소셜 미디어 공유 툴 {#section-eb575084a99647c3a9591f439f40b412}

eCatalog 뷰어는 소셜 공유 도구를 지원합니다. 기본 제어 막대에서 단추로 사용할 수 있으며 사용자가 클릭하거나 탭하면 공유 도구 모음으로 확장됩니다.
...
팝업 작업 모드에서는 기본 HTML 페이지를 사용하는 것이 좋습니다. 이 경우 이 매개 변수는 표준 IS-Viewers 배포의 하위 폴더 [!DNL eCatalogViewer.html] 내에 [!DNL html5/] 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일을 추가합니다.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 포함해야 합니다 [!DNL eCatalogViewer.js]. 이 [!DNL eCatalogViewer.js] 파일은 표준 IS-Viewers 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어 크기 설정
   ...
   최상위 CSS 클래스에 대해 절대 단위로 선언하거나 수정자를 사용하여 뷰어에 대한 정적 크기를 설정할 수 `.s7ecatalogviewer` `stagesize` 있습니다.
...
1. 뷰어를 초기화하는 중입니다.
   ...
   다음은 뷰어 인스턴스를 만들고, 필요한 최소 구성 옵션을 생성자에게 전달하고, 메서드를 호출하는 `init()` 예입니다. 이 예제에서는 뷰어 인스턴스 `eCatalogViewer` 를 가정합니다. `s7viewer` 은 자리 표시자 `DIV`이름입니다. `http://s7d1.scene7.com/is/image/` 는 이미지 제공 URL이며 자산 `Viewers/Pluralist` 은 다음과 같습니다.
   ...
   다음 코드는 크기가 고정된 eCatalog 뷰어를 포함하는 간단한 웹 페이지의 전체 예입니다.
