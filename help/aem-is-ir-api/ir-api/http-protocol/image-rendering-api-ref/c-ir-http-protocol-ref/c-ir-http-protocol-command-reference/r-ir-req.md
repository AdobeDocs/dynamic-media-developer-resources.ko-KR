...
<table id="simpletable_D04D41FBB03D4992B257CCBAD7EF0E7B"> 
 ...
 <tr class="strow"> 
  ...
  <td class="stentry"> <p><span class="codeph"> vignette::UserData </span>의 내용을 반환합니다. </p> </td> 
 </tr> 
</table>

자세한 설명에 별도로 언급되지 않는 한, 서버는 MIME 유형이 &lt;text/plain>인 텍스트 응답을 반환합니다.
...
<table id="table_A30296D29B5D43F1B5383A887252C6B4"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>문자열 </p> </td> 
   <td colname="col3"> <p> <span class="codeph"> 속성::수정자 또 </span> 는 카탈로그 항목이 아닌 경우 비어 있습니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>문자열 </p> </td> 
   <td colname="col3"> <p>수정 날짜/시간( <span class="codeph"> 카탈로그::TimeStamp </span> 또는 vignette 파일에서) </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>실제 </p> </td> 
   <td colname="col3"> <p><a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> 재료 해상도 </a> 단위(일반적으로 픽셀/인치)의 최대 개체 해상도입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> videnette.res.avg  </span> </p> </td> 
   <td colname="col2"> <p>실제 </p> </td> 
   <td colname="col3"> <p><a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> 재료 해상도 </a> 단위(일반적으로 픽셀/inc <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> 재료 해상도 </a>h)의 평균 개체 해상도. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> vignette.res.min  </span> </p> </td> 
   <td colname="col2"> <p>실제 </p> </td> 
   <td colname="col3"> <p><a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> 재료 해상도 </a> 단위(일반적으로 픽셀/인치)의 최소 개체 해상도. </p> </td> 
  </tr> 
  <tr> 
...
맵 데이터는 `wid=` 또는 `hei=` 로 조정되지 않거나 수정되지 않습니다. 응답 MIME 유형은 `<text/xml>`입니다.

응답 데이터는 HTML `<AREA>` 태그와 유사한 `<area>` 요소 세트가 들어 있는 `<map>` 요소로 구성됩니다.

각 `<area>` 요소에는 표준 `type=` 및 `coord=` 속성과 `name=` 속성이 포함되며, 비네팅 그룹 이름 또는 이름 경로를 지정합니다. 해당 개체 그룹의 마스크에 불연속 영역이 있는 경우 이름이 같은 여러 `<area>` 요소가 표시됩니다.

기본 특성 외에 작성된 경우 비네팅에서 추가 특성을 정의할 수 있습니다. 이러한 사용자 지정 속성은 객체 그룹 속성으로 정의됩니다. 사용자 지정 속성의 이름은 `map` 로 시작해야 합니다. `<area>` 요소에 포함될 예정입니다. 예를 들어 그룹 속성에 `map.href=http://www.scene7.com`이 포함되어 있으면 해당 `<area>` 요소에 `href="http://www.scene7.com"`가 포함됩니다.

비네트에 맵 데이터가 포함되지 않은 경우 빈 `<map>` 요소가 있는 XML 문서가 반환됩니다.

`object`

지정된 명령을 실행하고 남은 개체 선택(요청에서 마지막 `sel=` 또는 `obj=` 명령으로 선택한 그룹 또는 개체)에 의해 마스킹된 이미지를 반환합니다. 일반적으로 알파를 지원하는 이미지 형식과 함께 사용됩니다( [fmt=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c) 참조). 알파 기능을 지원하지 않는 이미지 형식을 사용하는 경우 마스크 외부의 영역은 검정색으로 표시됩니다.

`props`

지정된 명령을 실행하고 렌더링된 이미지가 아닌 비네팅 속성 및 그룹 또는 개체 속성을 반환합니다. 응답 구문 및 응답 MIME 유형에 대한 설명은 [속성](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-response-data/c-ir-properties.md#concept-e99f1a373eae4af9b41842ca0088ad3a)을 참조하십시오. `obj=` 또는 `sel=` 도 지정되지 않은 경우 기본 선택 사항이 적용됩니다( [ `obj=` ](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-obj.md#reference-31e7dac7931b4e0eb3c7589f120a1e6a) 참조).
...
<table id="table_F3ECF0584F6247A2A75C1CAFE1C57A4E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> 부울 </p> </td> 
   <td> <p>ICC 프로필이 회신 이미지에 포함되면 true입니다( <span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-iccembed.md#reference-47a433138c7c4b29b9b29871b2491a7f" type="reference" format="dita" scope="local"> iccEmbed= </a> </span> 참조). </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> image.iccProfile  </span> </p> </td> 
   <td> <p> 문자열 </p> </td> 
   <td> <p> 회신 이미지와 연결된 프로필의 바로 가기 이름( <span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-icc.md#reference-86a2fff3cef24982ad2063d977a16e06" type="reference" format="dita" scope="local"> icc= </a> </span> 참조). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p> 부울 </p> </td> 
   <td> <p> 회신 이미지에 경로 데이터가 포함된 경우 True입니다( <span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-pathembed.md#reference-dfff01079fc74dbd896362cc740d7f5f" type="reference" format="dita" scope="local"> pathEmbed= </a> </span> 참조). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>정수, 부울 </p> </td> 
   <td> <p> JPEG 품질 및 색상 플래그(<span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-qlt.md#reference-27b91c226eb241d0a14a29af3b3afdbd" type="reference" format="dita" scope="local"> qlt= </a> </span> 참조) </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> image.type  </span> </p> </td> 
   <td> <p> 문자열 </p> </td> 
   <td> <p> 회신 이미지에 대한 MIME 유형입니다( <span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c" type="reference" format="dita" scope="local"> fmt= </a> </span> 참조). </p> </td> 
  </tr> 
  <tr> 
...
`vignette::UserData`의 콘텐츠를 반환합니다. 서버는 `vignette::UserData`에 있는 `'??'`의 모든 항목을 줄 종료자( `<cr><lf>`)로 바꿉니다. 답글 형식은 응답 MIME 유형이 &lt;text/plain>으로 설정된 텍스트 데이터입니다.

URL 경로에 지정된 개체가 유효한 비네팅 맵 항목으로 확인되지 않거나 `vignette::UserData`이 비어 있는 경우 응답에는 라인 종료자( `CR/LF`)만 포함됩니다.
