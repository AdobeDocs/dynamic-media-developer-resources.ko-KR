...
사용자 지정 CSS를 만들 때 뷰어가 할당한다는 것을 기억하십시오 `.s7flyoutviewer` 클래스 이름을 컨테이너 DOM 요소로 지정합니다. 로 전달된 외부 CSS 파일을 사용하는 경우 `style=` 명령, 사용 `.s7flyoutviewer` 이 클래스는 CSS 규칙의 하위 선택기에서 상위 클래스로 사용됩니다. 웹 페이지에서 포함된 스타일을 수행하는 경우 컨테이너 DOM 요소의 ID로 이 선택기를 다음과 같이 분류하십시오.
...
반응형 디자인 CSS를 만드는 데 도움이 되도록 뷰어는 최상위 뷰어 컨테이너 요소에 동적으로 지정된 특수 CSS 클래스가 있는 CSS 마커를 지원합니다. 할당은 런타임 뷰어 크기 및 현재 장치에서 사용되는 입력 유형을 기반으로 합니다.
...
다른 픽셀 밀도를 사용하는 장치를 타깃팅하려면 CSS 미디어 쿼리를 사용합니다. 다음 미디어 쿼리 블록에는 고밀도 화면에 고유한 CSS가 포함됩니다.
...
CSS 마커를 사용하는 것은 반응형 디자인 CSS를 빌드하는 가장 유연한 방법입니다. 이유는 장치 화면 크기뿐 아니라 실제 뷰어 크기를 타겟팅할 수 있기 때문에 반응형 디자인 웹 페이지 레이아웃에 유용할 수 있습니다.
