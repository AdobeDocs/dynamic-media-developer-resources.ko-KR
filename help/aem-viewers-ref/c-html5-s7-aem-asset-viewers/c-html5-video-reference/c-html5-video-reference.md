---
title: 비디오
description: 비디오 뷰어는 H.264 형식으로 인코딩된 스트리밍 및 점진적 비디오를 재생하는 비디오 플레이어입니다. Dynamic Media Classic 또는 Adobe Experience Manager에서 Dynamic Media와 함께 제공됩니다.
keywords: 반응형
...
비디오 뷰어는 H.264 형식으로 인코딩된 스트리밍 및 점진적 비디오를 재생하는 비디오 플레이어입니다. Dynamic Media Classic 또는 Experience Manager에서 Dynamic Media와 함께 제공됩니다.
...
비디오 뷰어는 기본 시스템이 지원할 때마다 기본 구성으로 HTML5 스트리밍 비디오 재생을 HLS 형식으로 사용합니다. HTML5 스트리밍을 지원하지 않는 시스템에서는 뷰어가 HTML5 점진적 비디오 제공으로 대체됩니다.
...
비디오 뷰어는 기본 JavaScript 파일 및 도우미 파일 세트를 나타냅니다. 런타임 시 뷰어가 다운로드한 이 특정 뷰어, 에셋 및 CSS에서 사용하는 모든 뷰어 SDK 구성 요소에 단일 JavaScript이 포함됩니다.

IS-Viewers와 함께 제공되는 프로덕션 준비 HTML 페이지를 사용하여 팝업 모드에서 비디오 뷰어를 사용할 수 있습니다. 또는 내장된 모드로 뷰어를 사용할 수 있습니다. 이 모드에서는 문서화된 API를 사용하여 뷰어가 타겟 웹 페이지에 통합됩니다.
...
팝업 작업 모드에는 기본 제공 HTML 페이지를 사용하는 것이 좋습니다. [!DNL VideoViewer.html]이라고 하며 표준 IS-Viewers 배포의 [!DNL html5/] 하위 폴더 아래에 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일 추가.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 [!DNL FlyoutViewer.js]을(를) 포함해야 합니다. [!DNL FlyoutViewer.js] 파일은 표준 IS-Viewers 배포의 [!DNL html5/js/] 하위 폴더에 있습니다.
...
1. 뷰어 크기 설정
   ...
   CSS를 사용한 뷰어 스타일에 대한 자세한 내용은 [비디오 뷰어 사용자 지정](../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/c-html5-video-viewer-20-customizingviewer/c-html5-video-viewer-20-customizingviewer.md#concept-072a52b10b5f4c0789393dc6e2134c0e)을 참조하십시오.
...
1. 뷰어를 만들고 초기화하는 중입니다.
   ...
   다음은 뷰어 인스턴스를 만들고 필요한 최소 구성 옵션을 생성자에 전달하고 `init()` 메서드를 호출하는 예입니다. 이 예제에서는 `videoViewer`이(가) 뷰어 인스턴스이고, `s7viewer`이(가) 자리 표시자 `DIV`의 이름이고, [!DNL http://s7d1.scene7.com/is/image/]이(가) 이미지 제공 URL이고, [!DNL http://s7d1.scene7.com/is/content/]이(가) 비디오 서버 URL이고, [!DNL Scene7SharedAssets/Glacier_Climber_MP4]이(가) 자산이라고 가정합니다.
