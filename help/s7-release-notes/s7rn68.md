...
Adobe Experience Cloud Adobe Experience Manager 솔루션의 Adobe Scene7 Publishing System 6.8 부분에 대한 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보입니다.

## Scene7 Publishing System 6.8 릴리스 노트(2014년 5월) {#topic-4517aab837ea4d6d8204425958860e5a}

[!DNL Adobe Scene7 Publishing System]에 있는 Adobe Experience Manager 솔루션의 [!DNL Adobe Experience Cloud] 6.8 부분에 대한 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보입니다.
...
* [뷰어 5.0.1을 제공하는 Dynamic Media 이미지에 대한 릴리스 노트](s7rn68.md#section-3fc1a827bcc84c0488229d310086ad8e)
...
* 반응형 웹 디자인에 대한 지원 추가됨 - 중단점 및 페이지 레이아웃에 따라 동적으로 크기가 조정되는 반응형 HTML5 뷰어(이미지 세트, 비디오, 혼합 미디어, eCatalog, 스핀 세트, Swatchsets, 확대/축소)를 기반으로 하는 반응형 이미지
...
* HTML5 뷰어를 사용하여 CSS 입력 및 크기 마커를 사용하여 데스크탑 및 모바일 디바이스에서 디스플레이를 제어합니다.

이제 다음 기능이 지원됩니다.

* HTML5 비디오 뷰어와 혼합 미디어 뷰어에 대한 Adobe Analytics 비디오 보고서 통합입니다.
* 동일한 페이지에서 여러 HTML5 뷰어에 대한 이벤트 추적입니다.
* 모바일 장치에서 견본을 시각적으로 표현하기 위해 HTML5 확대/축소 뷰어 및 HTML5 혼합 미디어 뷰어에 `SetIndicator`이(가) 추가되었습니다.
...
* 이미지 맵을 외부 페이지에 연결하기 위한 HTML5 eCatalog Viewer의 JavaScript 템플릿.
* HTML5 eCatalog 뷰어 내의 페이지에 연결.
* HTML5 플라이아웃 뷰어에 대한 `stagesize`.
* 뷰어 다음에 함수를 트리거하기 위해 HTML5 뷰어에 대한 `initComplete` 처리기입니다.
...
* 차단 목록에 추가를 방지하려면 HTML에서 암호 찾기 이메일을 일반 텍스트로 변환합니다.
* 기본 역할을 제거했습니다.
* 서버 디렉토리 업로드(ftp)를 통해 Scene7 Publishing System에 컨텐츠를 업로드하고 URL을 업로드하는 기능 외에도 이제 HTTP POST를 사용하여 컨텐츠를 업로드할 수도 있습니다. HTTP POST 사용에 대한 자세한 내용은 [이미지 프로덕션 시스템 API](/help/aem-ips-api/c-overview.md)를 참조하십시오.
...
* 이미지 맵이 포함된 페이지가 있는 Internet Explorer에서 HTML5 eCatalog 뷰어 마우스 끌어서 놓기가 작동하지 않습니다.
* HTML5 Video Viewer는 Flash 플러그인에 대한 액세스를 차단합니다.
* HTML 5 뷰어는 Firefox 브라우저에서 인쇄를 방지합니다.
...
* HTML5 Video viewer Copy URL에 videoserverurl 수정자가 없습니다.
* `contenturl` 매개 변수는 게시 서버 HTML5 뷰어 미리 보기 URL에서 가져온 것입니다.
...
* 회사 설정을 초기화할 때 Scene7 Publishing System에서 API `<defaultValue>`에 중복 옵션 `<createMetadataField>`을(를) 보냅니다.

* 많은 폴더에 플래시 메모리가 부족하여 Flash 플러그인 및 Scene7 Publishing System이 충돌했습니다.
* 날짜/시간 사용자 정의 필드 값을 제거할 때 Scene7 Publishing System에서 IPS(Image Publishing System)로 요청을 보내지 않습니다.
...
* 모든 Scene7 Publishing System 웹 관리자 요청은 HTTP로 리디렉션됩니다.
...
* HTML 형식의 &quot;암호 찾기&quot; 이메일에 특수 문자가 인코딩되어 암호가 올바르게 표시되지 않습니다.
...
* 2014년 5월 Adobe Illustrator 및 Adobe InDesign 업데이트로 Scene7 FXG를 생성하는 플러그인과의 비호환성이 도입될 것으로 예상됩니다. 이러한 비호환성은 Scene7 Web-to-Print 솔루션 사용자에게 영향을 줍니다. Scene7 Web-to-Print를 사용하는 경우 Illustrator 및 InDesign을 이 Creative Cloud 업데이트로 업그레이드하지 마십시오. 대신 2014년 5월 이전 버전을 계속 사용하십시오. Adobe Inc.는 2014년 가을에 Scene7 FXG 생성과 최신 버전의 Illustrator 및 InDesign의 호환성이 복원될 것으로 예상하고 있습니다.
...
* 미리 알림: 이전 Scene7 릴리스부터 메타데이터 서버 및 관련 API 사용이 공식적으로 지원되지 않게 되었습니다.
* Scene7 Creative Suite 확장 프로그램(Flash 패널)은 2014년 말에 더 이상 사용되지 않습니다. 컨텐츠 수집 워크플로우의 추가 개발은 Adobe Experience Manager Dynamic Media 및 Creative Cloud에 중점을 두고 있습니다. Scene7의 경우 Scene7 Publishing System의 업로드 및 내보내기 기능이 유지되며, 대신 이 기능을 사용할 수 있습니다.
...
* HTML5 뷰어에서 전체 화면 모드로 표시되는 닫기 버튼입니다.

## 뷰어 5.0.1을 제공하는 Dynamic Media 이미지에 대한 릴리스 노트 {#section-3fc1a827bcc84c0488229d310086ad8e}

[https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html?lang=ko](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html?lang=ko) 보기
...
* `selectSwatch` 함수에 포함된 경우 `initViewer()` 메서드가 트리거되지 않는 문제가 해결되었습니다.
...
* 수정 사항: HTTPS 프로토콜이 플래시 재생을 사용하여 IE8에 대한 보안 경고를 생성합니다.
* 수정 사항: Safari 5에서 Flash 재생을 사용하여 전체 화면 모드에서 비디오 크기가 올바르게 지정되지 않습니다.
...
* 해결됨: HTML5 및 AS3 뷰어에 대한 HTTPS 지원.
* HTML5 Viewers 포함 코드에 반응형 지원이 추가되었습니다.
