...
Adobe Experience Cloud에 있는 Adobe Experience Manager 솔루션의 Adobe Scene7 Publishing System 6.9 부분에 대한 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보입니다.

## Scene7 Publishing System 6.9 릴리스 노트(2014년 10월) {#topic-1d3011ecca71488abe0d8156a9fc1b97}

의 최신 기능, 개선 사항 및 수정 사항을 설명하는 최신 릴리스 정보 [!DNL Adobe Scene7 Publishing System] 의 Adobe Experience Manager 솔루션 중 6.9 부분 [!DNL Adobe Experience Cloud].

* [Scene7 Publishing System 6.9 릴리스의 특징](s7rn69.md#section-2ed72b7902be4509876d81f25b5632ce)
...
* [Scene7 게시 시스템 6.9에 대한 버그 수정](s7rn69.md#section-86dc0e8eb85241dbb449221becb12888)
* [더 이상 지원되지 않는 기능](s7rn69.md#section-b061ad3d170a45ee8e694f997fd5d8ef)

## Scene7 Publishing System 6.9 릴리스의 특징 {#section-2ed72b7902be4509876d81f25b5632ce}
...
* **반응형 디자인** - 각 중단점에 대해 서로 다른 이미지 사전 설정 또는 수정자를 정의하여 응답형 이미징 경험의 품질을 더욱 최적화합니다.
* **뷰어 사용자 지정** - 이제 뷰어의 스타일 및 비헤이비어를 즉시 동적으로 설정하고 수정할 수 있습니다.
* **Visual Configurator 보안 미리 보기** - 개인화된 경험을 프로덕션에 푸시하기 전에 시각적 렌더링을 안전하게 미리 보고 테스트합니다.
* **SSO(Single Sign-On) 기능** - 이제 Adobe Experience Cloud에서 Scene7을 원활하게 시작하고 Adobe ID을 사용하여 Scene7에 로그인할 수 있습니다.

   Scene7에서 사용할 수 있도록 SSO를 설정하려면 Adobe 기술 지원에 문의하십시오. Experience Cloud과 Scene7 계정을 함께 연결할 수 있도록 Adobe Experience Cloud 조직 이름, Scene7 회사 이름 및 지리적 지역 정보를 준비하십시오.

## 이미징 {#section-98a61ea9a09940639f357f36969d6e55}

* **반응형 지원 향상** - 각 중단점에 대해 서로 다른 이미지 사전 설정 또는 수정자를 정의하여 응답형 이미징 경험의 품질을 더욱 최적화합니다. 다음을 참조하십시오 [응답형 정적 이미지 라이브러리 사용](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html).

## 비디오 {#section-69101b28504543a7ae8a9346d381f913}

* 이제 모든 종횡비와 함께 작동하며 모바일, 태블릿 및 데스크톱에 전송할 비디오를 만드는 새로운 &quot;적응형 비디오&quot; 인코딩 사전 설정입니다. 이 새로운 사전 설정으로 인코딩된 업로드된 소스 비디오는 고정된 높이로 설정되고 너비는 비디오의 종횡비를 유지하기 위해 자동으로 조절됩니다. [비디오 파일 인코딩을 위한 비디오 사전 설정](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html?lang=en#setup)을 참조하십시오.
* 인코딩으로 탄력적인 확장성과 성능을 개선했습니다.
* 이제 즉시 사용 가능한 비디오 캡션 기능 `Universal_HTML5_MixedMedia_dark` 및 `Universal_HTML5_MixedMedia_light` 뷰어. [비디오에 캡션 추가](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html#video)를 참조하십시오.
...
* 렌더링 서버 요청에 대한 보안 테스트 - 이제 핵심 이미지 서비스 자산 및 비디오 자산에 대한 보안 테스트가 비네팅도 포함하도록 확장되었습니다. 다음을 참조하십시오 [공개하기 전에 자산 테스트](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish).
* 무효화 CDN 캐시 기능 모니터링. CDN 무효화 템플릿을 선택한 후( 참조) [일반 설정](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings)), 및 **[!UICONTROL CDN 무효화]** 요청이 수행되면 이제 캐시가 지워지는 데 걸리는 시간에 대한 예상과 함께 표시기가 나타납니다.

## 뷰어 {#section-fc0577722d0b431dae3506c8d4e5366f}

**Scene7 HTML Viewer 5 SDK 2.8.1의 새로운 기능, 개선 사항 및 버그 수정**

일반

* 을 사용하는 구성 요소 `insertBefore()` API 이제 사용 `appendChild()` API.
* 에 대한 지원이 추가됨 `setCSS()`, `setModifier()`, 및 `dispose()` 메서드를 사용합니다.
* 에 대한 지원이 추가됨 `getWidth()`, 및 `getHeight()` 메서드를 사용합니다.
* 에 대한 지원이 추가됨 `setPortrait()` 및 `setLandscape ()` 메서드를 사용합니다.
...
* 방향별 반환 지원이 추가되었습니다. `MediaSetDesc` 개체.
...
* 고정 - 다음과 같은 경우 이미지가 오른쪽으로 이동함 `DIV` 이(가) `text-align:center` 스타일입니다.
...
**Dynamic Media 이미지 제공 뷰어 5.1.1의 새로운 기능, 개선 사항 및 버그 수정**
...
컴퓨터가 스테이징 서버에 액세스하도록 설정한 후 웹 사이트를 확인하여 업그레이드를 테스트할 수 있습니다. 기본 제공 HTML 5 뷰어를 사용하는 고객의 경우 Adobe의 표준 스테이징 서버 및 s7is1-preview-staging.scene7.com에 대해 테스트하는 것이 좋습니다.
...
* 사용할 뷰어를 변경했습니다. `Track requests` 대신 `Page requests` Adobe Analytics 추적용
...
* HTML5 eCatalog의 기본 페이지 스크롤에 대한 지원을 추가했습니다.
* 을 사용하여 좁은 이미지를 표시하는 데 대한 지원을 추가했습니다. `highlightmode` HTML5 플라이아웃 뷰어에 대한 수정자
* 를 사용하여 이미지 업스케일링을 제어하는 지원을 추가했습니다. `zoomfactor` HTML5 플라이아웃 뷰어에 대한 수정자입니다.
...
* HTML 5 eCatalog 뷰어가 iPhone에 PageIndicator를 표시하지 않습니다.
...
* HTML5 MixedMedia 뷰어: 첫 번째 에셋이 2dspinset인 경우 응답형 모드에서 뷰어가 축소됩니다.
...
## Scene7 게시 시스템 6.9에 대한 버그 수정 {#section-86dc0e8eb85241dbb449221becb12888}

* 내보낸 에셋의 총 수는 내보내기 작업 로그 세부 사항의 원래 수보다 한 개 더 큽니다.
* 일괄처리 집합 사전 설정에서 생성된 1d-spin에서는 회전 집합 편집기를 열 때만 경고가 표시됩니다.
* 곧바로 사용 가능한 HTML5 비디오 뷰어 사전 설정을 미리 볼 수 없습니다.
* Scene7 출판 시스템은 규칙이 다시 작성되더라도 그룹을 계속 기억했습니다.
* Scene7 Publishing System에 반복에 대한 특정 정규 표현식 코드가 제대로 표시되지 않았습니다.
...
* 메타데이터 서버 및 유사성 검색은 채택률이 낮아 2015년에 더 이상 사용되지 않습니다. 공식적인 사용 중단 목표일이 발표되면 Adobe 기술 지원에서 해당 고객에게 알렸습니다.
