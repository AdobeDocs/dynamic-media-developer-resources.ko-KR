...
`attribute::UseLastModified`이 설정되면 요청에 관련된 모든 자료의 `catalog::TimeStamp` 및 `vignette::TimeStamp` 값 중 가장 최근 값이 HTTP 응답에서 마지막 수정 헤더로 반환됩니다.
...
문자열 형식의 날짜/시간 값의 모든 요소가 있어야 합니다. 날짜/시간 값의 형식이 올바로 지정되지 않으면 무시되고 *catalog*.ini 파일의 수정 시간이 대신 사용됩니다.
