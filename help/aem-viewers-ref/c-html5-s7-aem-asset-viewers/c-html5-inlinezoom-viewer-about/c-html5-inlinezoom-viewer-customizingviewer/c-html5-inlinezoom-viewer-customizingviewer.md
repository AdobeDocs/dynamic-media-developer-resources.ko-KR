...
사용자 지정 CSS를 만들 때에는 뷰어가 `.s7flyoutviewer` 클래스 이름을 컨테이너 DOM 요소로 지정합니다. 로 전달된 외부 CSS 파일을 사용하는 경우 `style=` 명령, 사용 `.s7flyoutviewer` 이 클래스는 CSS 규칙의 하위 선택기에서 상위 클래스로 사용됩니다. 웹 페이지에서 포함된 스타일을 수행하는 경우 컨테이너 DOM 요소의 ID로 이 선택기를 다음과 같이 분류하십시오.
...
반응형 디자인 CSS를 만드는 데 도움이 되도록 뷰어는 최상위 뷰어 컨테이너 요소에 동적으로 할당된 특수 CSS 클래스를 CSS 마커를 지원합니다. 할당은 런타임 뷰어 크기와 현재 장치에서 사용되는 입력 유형을 기반으로 합니다.
...
CSS 마커를 사용하는 것은 반응형 디자인 CSS를 빌드하는 가장 유연한 방법입니다. Target을 사용하면 장치 화면 크기뿐만 아니라 실제 뷰어 크기를 지정할 수 있으므로 반응형 디자인 웹 페이지 레이아웃에 유용할 수 있습니다.
