...
seo-description: 현재 활성화된 모든 작업을 가져옵니다.
...
| ` *`jobHandle`*` | `xsd:string` | 아니요 | 그 일의 손잡이다. |
...
이 코드 샘플은 IPS에서 실행 중인 회사의 모든 활성 작업을 반환합니다. 이 경우 IPS 예약 코디네이터가 활성 작업이 실행되지 않고 비활성화되어 응답이 흔치 않습니다. 정상적인 상황에서 응답으로 많은 활성 작업이 반환됩니다.
