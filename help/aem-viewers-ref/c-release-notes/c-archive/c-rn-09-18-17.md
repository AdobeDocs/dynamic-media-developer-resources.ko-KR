...
* 다음과 같은 경우 사용자는 마우스를 사용하여 페이지를 켤 수 없습니다. `frametransition=turn`.
* `ImageMapEffect.mapTips=1` 터치 지원 장치에서 마우스 클릭으로 스크립트 오류가 발생했습니다.

## 5.8.1의 새로운 기능 및 향상된 기능 {#section-f55c5ff7f6d0406c8739de4981dd7195}

* 기본 확대/축소, 플라이아웃, 혼합 미디어, 회전, 확대/축소, 비디오, 회전 메뉴, 대화형 이미지 및 대화형 비디오에서의 키보드 접근성.
* 비디오 뷰어, 혼합 미디어 뷰어 및 대화형 비디오 뷰어에 비디오 버퍼링 아이콘을 추가했습니다.
* 값 `native` / `playback` 수정자는 사용되지 않습니다. 새 값을 사용하십시오. `progressive` 대신,

* 다음에 대한 HTTPS 스트리밍 재생 지원이 추가되었습니다. [!DNL Dynamic Media] 비디오입니다.
...
* 사용자가 키보드 탭 이동을 통해 비디오 컨트롤에 초점을 맞출 수 없습니다.
* eCatalog 뷰어에서 단일/이중 페이지 레이아웃이 일부 장치에 잘못 표시되었습니다.
...
* 포스터 이미지에 다음이 발생할 수 있습니다. `max size` 오류. 이미지 제공 게시의 제한 설정을 늘립니다.
