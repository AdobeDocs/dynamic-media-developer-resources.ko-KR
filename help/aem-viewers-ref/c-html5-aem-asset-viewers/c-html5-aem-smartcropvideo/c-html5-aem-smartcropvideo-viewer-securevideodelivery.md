...
* 포함 코드가 있는 Dynamic Media 비디오를 게시하는 경우 `SmartCropVideoPlayer.ssl` 포함 코드 조각의 다른 뷰어 구성 매개 변수 목록. 예를 들어 HTTPS 비디오 전송을 강제하려면 다음을 추가합니다 `&SmartCropVideoPlayer.ssl=on` 다음 예제와 같이:
