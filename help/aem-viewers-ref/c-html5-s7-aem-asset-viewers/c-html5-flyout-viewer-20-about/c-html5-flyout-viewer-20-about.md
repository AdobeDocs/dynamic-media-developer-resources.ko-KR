---
description: 플라이아웃 뷰어는 이미지 뷰어입니다. 사용자가 활성화한 플라이아웃 보기에 축소된 버전이 표시된 정적 이미지가 표시됩니다. 이 뷰어는 이미지 세트와 연동하며 견본을 사용하여 탐색합니다. 데스크톱 및 모바일 장치에서 작동하도록 설계되었습니다.
keywords: responsive
seo-description: 플라이아웃 뷰어는 이미지 뷰어입니다. 사용자가 활성화한 플라이아웃 보기에 축소된 버전이 표시된 정적 이미지가 표시됩니다. 이 뷰어는 이미지 세트와 연동하며 견본을 사용하여 탐색합니다. 데스크톱 및 모바일 장치에서 작동하도록 설계되었습니다.
seo-title: 플라이아웃
...
플라이아웃 뷰어는 이미지 뷰어입니다. 사용자가 활성화한 플라이아웃 보기에 축소된 버전이 표시된 정적 이미지가 표시됩니다. 이 뷰어는 이미지 세트와 연동하며 견본을 사용하여 탐색합니다. 데스크톱 및 모바일 장치에서 작동하도록 설계되었습니다.
...
플라이아웃 뷰어는 런타임에 뷰어가 다운로드한 기본 JavaScript 파일 및 도우미 파일 집합(단일 JavaScript는 이 특정 뷰어에서 사용하는 모든 뷰어 SDK 구성 요소, 자산, CSS)을 나타냅니다

플라이아웃 뷰어는 포함된 용도로만 사용되며, 이것은 문서화된 API를 사용하여 웹 페이지에 통합됨을 의미합니다. 플라이아웃 뷰어에 대해 바로 사용할 수 있는 웹 페이지가 없습니다.
...
웹 페이지마다 뷰어 비헤이비어에 대한 요구 사항이 다릅니다. 웹 페이지에는 정적 페이지 레이아웃이 있거나, 다른 장치 또는 서로 다른 브라우저 창 크기에 대해 다르게 표시되는 반응형 디자인을 사용할 수 있습니다. 이러한 요구 사항을 충족하기 위해 뷰어는 두 가지 기본 작업 모드를 지원합니다. 고정 크기 임베드 및 반응형 디자인 임베드.
...
플라이아웃 뷰어와 함께 반응형 디자인 임베드 모드를 사용할 때는 매개 변수를 사용하여 기본 보기 이미지에 대한 명시적 중단점을 지정해야 `imagereload` 합니다. 웹 페이지 CSS에서 지정한 뷰어 폭 중단점과 중단점을 일치시키는 것이 좋습니다.
...
1. 뷰어 크기 설정
   ...
   CSS를 [사용한 뷰어](../../c-html5-s7-aem-asset-viewers/c-html5-flyout-viewer-20-about/c-html5-flyout-viewer-20-customizingviewer/c-html5-flyout-viewer-20-customizingviewer.md#concept-82f8c71adbe54680a0c2f83f81e5f451) 스타일 지정에 대한 자세한 내용은 플라이아웃 뷰어 사용자 지정을 참조하십시오.
...
1. 뷰어를 만들고 초기화합니다.
   ...
   다음은 필요한 최소 구성 옵션을 생성자에게 전달하고 메서드를 호출하는 뷰어 인스턴스를 만드는 `init()` 예입니다. 이 예제에서는 뷰어 인스턴스 `flyoutViewer` 를 가정합니다. `s7viewer` 은 자리 표시자 `DIV`이름입니다. `http://s7d1.scene7.com/is/image/` 는 이미지 제공 URL; 자산 `Scene7SharedAssets/ImageSet-Views-Sample` 은 다음과 같습니다.
   ...
   다음 코드는 크기가 고정된 플라이아웃 뷰어를 포함하는 간단한 웹 페이지의 완전한 예입니다.
