...
일반적인 JPEG 인코더에서 사용하는 색도 다운샘플링을 비활성화하려면 *`chroma`* 플래그를 설정하십시오. 이 설정은 에지가 밝기가 아닌 색조의 변화에 의해 정의될 때 이미지에서 에지의 인지된 선명도를 증가시킬 수 있다. 이 플래그를 설정하면 파일 크기가 약간 증가할 수 있습니다. 텍스트가 약간 흐리게 보이는 경우 이 설정을 실험해 보십시오.
