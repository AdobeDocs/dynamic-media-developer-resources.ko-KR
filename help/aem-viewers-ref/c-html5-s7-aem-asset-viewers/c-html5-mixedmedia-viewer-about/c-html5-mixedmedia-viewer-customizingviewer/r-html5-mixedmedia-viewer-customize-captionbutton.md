---
title: 캡션 단추
description: 선택 캡션 표시를 켜거나 끕니다. caption 매개 변수를 지정하지 않으면 표시되지 않습니다. CSS를 사용하여 이 단추의 크기를 조정하고, 스킨을 지정하고, 이 단추가 포함된 컨트롤 막대를 기준으로 이 단추를 배치할 수 있습니다.
...
선택 캡션 표시를 켜거나 끕니다. caption 매개 변수를 지정하지 않으면 표시되지 않습니다. CSS를 사용하여 이 단추의 크기를 조정하고, 스킨을 지정하고, 이 단추가 포함된 컨트롤 막대를 기준으로 이 단추를 배치할 수 있습니다.
...
>[!NOTE]
>
>이 단추는 `state` 특성 선택기와 선택한 특성 선택기를 지원하며, 이 선택기는 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히 `selected='true'`은(는) 캡션이 표시되는 상태에 해당하며, `selected='false'`은(는) 캡션이 숨겨지는 경우에 사용됩니다.
...
예 - 28 x 28픽셀로, 맨 위에서 4픽셀, 컨트롤 막대의 오른쪽 가장자리에서 68픽셀로 배치된 자막 단추를 설정합니다. 마지막으로, 을 선택하거나 선택하지 않았을 때 4개의 서로 다른 단추 상태 각각에 대해 다른 이미지를 표시합니다.
