...
사진 컨텐츠가 포함된 PTIFF 파일은 JPEG으로 인코딩하는 것이 좋습니다(지정 `-jpegcompress`). 컴퓨터에서 생성한 콘텐츠는 손실 없는 압축으로 얻을 수 있습니다. `-deflatecompress` 또는 `-lzwcompress`). 색상 변환이나 픽셀 유형 변환이 필요하지 않으면 JPEG 소스 이미지 데이터가 디코딩 없이 PTIFF로 전송되어 품질 저하를 방지할 수 있습니다. 이 경우 지정된 압축 옵션은 낮은 해상도 피라미드에만 적용됩니다.

큰 이미지를 변환하지 않는 경우 사용할 메모리 양을 제어하는 매개 변수를 설정할 필요가 없습니다. 하지만 만약 그렇다면, `ic` 더 많은 메모리 사용 `-maxmem` 설정에 대해서는 아래에 설명되어 있습니다. 필요한 메모리 양을 계산하기 위한 좋은 경험의 법칙은 이미지 너비와 이미지 높이를 채널 수와 곱하는 것입니다. 예를 들어, 알파 곱이 3인 RGB 이미지의 경우 4입니다. 또한 채널이 최종 결과의 8배 대신 구성 요소당 16비트인 경우
...
성공할 경우 0입니다. 오류가 발생하면 0이 아닌 값이 반환되고 오류 세부 정보가 로 전송됩니다 `stderr`.
...
<table id="table_02011C7C076745A8BF4378B22C48C8A3"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>JPEG 인코딩을 사용합니다. 다음 경우 무시됨 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 알파 데이터를 포함합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>JPEG 색상-다운샘플링을 비활성화합니다(색상 텍스트 및 그래픽의 품질을 향상시킬 수 있음). CMYK 또는 회색 명암 출력 이미지에는 영향을 주지 않습니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>하위 샘플링된 피라미드 레벨에 언샵 마스킹을 적용합니다. 자세한 내용은 <a href="../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-command-reference/r-op-usm.md#reference-51ac75adadfe4346ab60953192d0a1aa" type="reference" format="dita" scope="local"> op_usm= </a> 자세한 내용 (전체 해상도 이미지에는 적용되지 않습니다.) </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>인쇄 해상도(dpi) <span class="codeph"> <span class="varname"> destFile </span> </span>; 지정하지 않으면 <span class="codeph"> srcFile </span> 에 복사됨 <span class="codeph"> <span class="varname"> destFile </span> </span>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -autocrop &lt; <span class="varname"> 모퉁이 </span>&gt; &lt; <span class="varname"> 모드 </span>&gt; &lt; <span class="varname"> 허용 </span>&gt; &lt; <span class="varname"> infoFile </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>자르기 사각형을 계산하여 단색 배경을 최소화합니다. 자동 자르기 알고리즘으로 전체 이미지가 잘리는 경우 자르기 정보가 출력되지 않습니다. </p> <p>이미지를 변환하지 않고 자르기 사각형을 계산하려면 <span class="codeph"> -autocrop </span> 사용 안 함 <span class="codeph"> -convert </span> 그리고 <span class="codeph"> <span class="varname"> destFile.</span> </span></p>
...
<p><i><b>모퉁이</b></i> - ul | url | ll | lr </p>
   ...
   <p><i><b>허용</b></i> - 일치 허용치. 실수 값 0.0에서 1.0까지 일치하는 픽셀 구성 요소 값에 대한 허용치를 지정합니다. 정확히 일치하려면 0으로 설정합니다.</p>
...
<p>  
  ...
  <tr> 
   ...
   <td colname="col2"> <p>가능한 경우 다음 위치에서 XMP 메타데이터 복사 <span class="codeph"> <span class="varname"> sourceFile </span> </span> to <span class="codeph"> <span class="varname"> destFile </span> </span> 수정 없이. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -embedColorProfile </span> </p> </td> 
   <td colname="col2"> <p> ICC 색상 프로파일 포함 <span class="codeph"> <span class="varname"> destFile </span> </span>사용 가능한 경우(기본적으로 프로필이 포함되지 않음). </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -imageprofile &lt; <span class="varname"> 파일 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>ICC 프로필 파일의 경로 및 이름입니다. 의 색상 공간을 정의합니다 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 및 은 해당 픽셀 유형과 일치해야 합니다. 에 프로필이 포함되지 않은 경우에만 지정해야 합니다. <span class="codeph"> <span class="varname"> sourceFile </span> </span>: 포함된 프로필을 재정의합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -viewprofile &lt; <span class="varname"> 파일 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>ICC 프로필 파일의 경로 및 이름입니다. 픽셀 유형 및 색상 공간 정의 <span class="codeph"> <span class="varname"> destFile </span> </span>. IC가 다음 경우에 이 프로필로 변환됩니다 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 에는 내장 프로필이 있거나 <span class="codeph"> -imageprofile </span> 도 지정됩니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> CMYK에서 RGB으로 자동 변환을 비활성화합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> - forceJPEGDecompress </span> </p> </td> 
   <td colname="col2"> <p>JPEG 입력 이미지의 강제 복호화 및 재인코딩. </p> <p> <b>주의:</b> 이 옵션을 적용하면 이미지 품질이 저하될 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>기존 덮어쓰기 허용 <span class="codeph"> <span class="varname"> destFile </span> </span>. 기본적으로 파일 이름에 숫자 접미사가 추가되어 덮어쓰기를 방지합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>로그 파일의 경로 및 이름(기본값은 임) <span class="codeph"> stdout </span>). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -maxmempercent &lt; <span class="varname"> 퍼센트 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>메모리 사용 제한. 기본값은 실제 메모리의 25%입니다. 둘 중 어느 것도 아니면 <span class="codeph"> maxmem </span> 아니요 <span class="codeph"> maxmempercent </span> 명시적으로 설정된 경우 maxmempercent 기본값을 사용합니다. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_1EB2B60993D34B1887275EA4E3491401"> 
 <thead> 
  <tr> 
   ...
   <th class="entry"> <p> <b> 픽셀 유형</b> <b> 비트/찬</b> </p> </th> 
...
모든 이미지를 *`srcFolder`* JPEG으로 인코딩된 피라미드 TIFF 및 *`destFolder`*:

`ic -convert -jpegcompress -jpegquality 90 -overwrite -continueOnError srcFolder destFolder`

모든 이미지를 *`srcFolder`*. JPG 파일의 부호화된 이미지 데이터는 전체 해상도의 손실 없는 LZW 압축과 JPG이 아닌 모든 입력 파일의 전체 출력 이미지에 사용됩니다. 픽셀 유형, 포함된 색상 프로필, XMP 메타데이터 등 가 유지 관리됩니다.
