...
다음은 재생 중에 사운드가 비활성화되도록 비디오 플레이어를 사용자 정의할 수 있는 방법의 예입니다. 뷰어의 포함 코드에 다음 코드를 추가합니다.
...
위의 코드 예에서 볼륨 수준은 `mutableVolume` 구성 요소의 `0`으로 설정됩니다. 그런 다음 동일한 구성 요소가 비활성화되므로 최종 사용자가 해당 구성 요소를 사용할 수 없습니다.
