...
반복 가능한 텍스처가 비네팅 오브젝트에 적용되어 텍스처 기준점이 오브젝트의 텍스처 원점에 위치합니다. 비네팅 오브젝트에 디컬된 이미지가 적용되어 디컬된 고정점이 오브젝트의 디털 원점에 위치하게 됩니다. 벽 테두리의 경우 x 값만 사용됩니다.y 값은 무시됩니다.

## 속성 {#section-bc4bc8b897c64535b88681e57d72942f}

두 정수(쉼표로 구분)입니다. 이미지의 왼쪽 위 모서리를 기준으로 한 픽셀 오프셋. 단색 `catalog::Alignment=3` 및 캐비닛 재질이 있는 경우 무시됩니다.

## 기본값 {#section-b7ccc419a356415294706cd295ae96c9}

필드가 비어 있거나 없는 경우, 반복 가능한 텍스처 재질에 대한 이미지의 왼쪽 위 모서리(0,0)가 사용되거나, 디칼 재질의 경우 이미지 중심이 사용됩니다.
