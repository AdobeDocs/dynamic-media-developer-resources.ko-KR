...
IS-Viewer와 함께 제공되는 프로덕션 준비 HTML 페이지나 포함된 모드에서 문서 API를 사용하여 대상 웹 페이지에 통합되는 팝업 모드에서 eCatalog Viewer를 사용할 수 있습니다.
...
이 뷰어는 [키보드 액세스 가능성 및 탐색](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861)에 설명된 대로 완전히 키보드 액세스 가능합니다.
...
팝업 작업 모드에 기본 HTML 페이지를 사용하는 것이 좋습니다. 이 경우, [!DNL eCatalogViewer.html] 이라고 하며, 표준 IS-Viewers 배포의 [!DNL html5/] 하위 폴더 내에 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일 추가.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 [!DNL eCatalogViewer.js]를 포함해야 합니다. [!DNL eCatalogViewer.js] 파일은 표준 IS-Viewers 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어 크기 설정
   ...
   `.s7ecatalogviewer` 최상위 CSS 클래스에 대해 절대 단위로 선언하거나 `stagesize` 한정자를 사용하여 뷰어의 정적 크기를 설정할 수 있습니다.
