---
description: 목차는 기본 컨트롤 막대에 있는 단추입니다. 활성화되면 페이지 인덱스 및 레이블 목록이 있는 드롭다운 패널이 나타납니다.
seo-description: 목차는 기본 컨트롤 막대에 있는 단추입니다. 활성화되면 페이지 인덱스 및 레이블 목록이 있는 드롭다운 패널이 나타납니다.
seo-title: 목차
...
목차는 기본 컨트롤 막대에 있는 단추입니다. 활성화되면 페이지 인덱스 및 레이블 목록이 있는 드롭다운 패널이 나타납니다.
...
예 - 기본 컨트롤 막대 왼쪽에서 4픽셀, 왼쪽에서 43픽셀로 배치된 목차 단추를 설정합니다.크기는 28 x 28픽셀이며 서로 다른 4가지 단추 상태에 대해 서로 다른 이미지가 표시됩니다.
...
<table id="table_A18B6978EC304C378F5FE92DD44D138D"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 패널 경계와 내용 간의 내부 오프셋. </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>드롭다운 목록 항목은 `state` 속성 선택기를 지원합니다. 속성 선택기를 사용하여 마우스로 가리키거나 선택한 항목 상태를 적용하는 데 사용할 수 있습니다.

예 - Helvetica 14픽셀 글꼴과 19픽셀 높이의 드롭다운 항목을 설정합니다. 항목의 배경에는 어두운 회색이 있고 선택할 경우 밝은 회색이 있습니다.
...
페이지 인덱스를 표시하는 요소는 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
<table id="table_FAA5072E4AAC48F4BE00B05D87FD9827"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 최소 너비 </span> </p> </td> 
   <td colname="col2"> <p> 최소 요소 너비. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> max-width </span> </p> </td> 
   <td colname="col2"> <p> 최대 요소 너비. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 패딩 오른쪽 </span> </p> </td> 
   <td colname="col2"> <p> 페이지 색인과 페이지 레이블 사이의 거리입니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]
>
>CSS 클래스에 `display:none` 대해 설정하여 페이지 색인을 완전히 숨길 수 `s7index` 있습니다.

예 1 - 최소 너비 40픽셀, 최대 너비 70픽셀, 오른쪽 5픽셀 여백을 사용하여 페이지 인덱스를 설정합니다.
...
예 - 최소 너비 40픽셀과 최대 너비 240픽셀의 페이지 인덱스를 설정합니다.
...
드롭다운 패널에서 세로로 맞출 수 있는 항목 수가 많고 시스템이 데스크탑인 경우 구성 요소는 패널 오른쪽에 세로 스크롤 막대를 렌더링합니다. 스크롤 막대 영역의 모양은 다음 CSS 클래스 선택기로 제어됩니다.
...
<table id="table_D34A63AAE6324699ABDCC08355D33035"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 패널 영역의 위쪽에서 세로 스크롤 막대 오프셋입니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 패널 영역의 오른쪽 가장자리에서 벗어난 가로 스크롤 막대 오프셋입니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
예 - 세로 28픽셀이고 반투명 회색 배경이 있는 스크롤 막대 트랙을 설정합니다.
...
스크롤 막대 축소판 표시는 스크롤 트랙 영역 내에서 세로로 이동합니다. 세로 위치는 구성 요소 논리로 제어됩니다. 그러나 축소판 높이는 콘텐츠의 양에 따라 동적으로 변경되지 않습니다. 다음 CSS 클래스 선택기를 사용하여 축소판 높이와 기타 측면을 구성할 수 있습니다.
...
>[!NOTE]
>
>Thumb는 `state` 속성 선택기를 지원합니다. 이 `up`선택기는 `down`, `over``disabled` 및 thumb 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다.

예 - 28 x 45픽셀인 스크롤 막대 축소판을 설정하고 위쪽과 아래쪽에 10픽셀 여백을 가지며 각 상태에 대해 서로 다른 아트워크를 가집니다.
...
>[!NOTE]
>
>Button은 `state` 속성 선택기를 지원합니다. 이 `up`선택기는 `down`, `over``disabled` 및 버튼 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다.
