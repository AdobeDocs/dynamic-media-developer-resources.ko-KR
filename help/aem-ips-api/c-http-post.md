...
업로드 작업은 일반적인 `jobHandle`을(를) 사용하여 동일한 작업에 처리를 상호 연결하는 하나 이상의 HTTP POST로 구성됩니다. 각 요청은 `multipart/form-data` 인코딩되어 다음 양식 부분으로 구성됩니다.
...
`uploadParams` 매개 변수는 동일한 작업의 일부로서 개별 파일에 대해 변경할 수 있지만 그렇지 않을 수 있습니다. 전체 작업에 동일한 `uploadParams` 매개 변수를 사용합니다.

새 업로드 작업에 대한 초기 POST 요청은 `jobName` 매개 변수를 지정해야 합니다. 고유 작업 이름을 사용하여 후속 작업 상태 폴링 및 작업 로그 쿼리를 간소화할 수 있습니다. 동일한 업로드 작업에 대한 추가 POST 요청은 초기 요청에서 반환된 `jobHandle` 값을 사용하여 `jobName` 대신 `jobHandle` 매개 변수를 지정해야 합니다.

업로드 작업에 대한 최종 POST 요청은 이 작업에 대해 향후 파일이 POST가 되지 않도록 `endJob` 매개 변수를 true로 설정해야 합니다. 이렇게 하면 모든 POSTed 파일을 인제스트한 직후에 작업을 완료할 수 있습니다. 그렇지 않으면 30분 이내에 추가 POST 요청을 받지 못할 경우 작업이 시간 초과됩니다.

## UploadPOST 응답 {#section-421df5cc04d44e23a464059aad86d64e}

성공적인 POST 요청의 경우 XSD에서 지정하는 대로 응답 본문은 XML `uploadPostReturn` 문서가 됩니다.
...
반환된 `jobHandle`은 동일한 작업에 대한 후속 POST 요청에 대해 `uploadPostParams`/ `jobHandle` 매개 변수로 전달됩니다. 이 기능을 사용하여 작업 상태를 `getActiveJobs` 작업으로 폴링하거나 작업 로그를 `getJobLogDetails` 작업으로 쿼리할 수도 있습니다.
