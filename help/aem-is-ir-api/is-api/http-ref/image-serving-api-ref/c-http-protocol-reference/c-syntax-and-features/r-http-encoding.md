...
그렇지 않으면 표준 HTTP 인코딩 규칙이 적용됩니다. HTTP 사양은 안전하지 않은 문자뿐만 아니라 `<return>` 및 같은 모든 컨트롤 문자를 인코딩해야 `<tab>`합니다. 문자의 URL 인코딩은 &quot;%&quot; 기호로 구성되며, 그 뒤에 ISO-라틴 코드 포인트의 두 자리 16진수 표현(대/소문자 구분 안 함)이 옵니다. 안전하지 않은 문자와 코드 포인트는 다음과 같습니다.
