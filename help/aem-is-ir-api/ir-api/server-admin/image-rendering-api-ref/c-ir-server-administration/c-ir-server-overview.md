...
* Java 패키지는 이미지 제공 [!DNL Platform Server]과(와) 함께 배포되고 클라이언트 연결, 캐싱, 재질 카탈로그를 관리합니다.
* 기본 코드 모듈은 이미지 서버용 확장 라이브러리로 배포되고 핵심 이미지 렌더링 기능을 구현합니다.

두 구성 요소를 통칭하여 *렌더링 서버*&#x200B;라고 합니다.

이미지 렌더링은 이미지 제공과 많은 서버 기능을 공유하며, 모든 옵션은 구성 파일을 편집하여 구성됩니다. 기본 카탈로그([!DNL default.ini]) 또는 특정 재질 카탈로그에서 추가 구성 특성을 제공합니다. 자세한 내용은 자재 카탈로그 를 참조하십시오.
