---
description: eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, 사용자는 eCatalog를 사용하여 추가 사용자 인터페이스 요소 또는 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 사용자는 모든 페이지를 확대하여 자세히 볼 수 있습니다.
keywords: responsive
seo-description: eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, 사용자는 eCatalog를 사용하여 추가 사용자 인터페이스 요소 또는 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 사용자는 모든 페이지를 확대하여 자세히 볼 수 있습니다.
seo-title: eCatalog
...
eCatalog 뷰어는 스프레드 또는 페이지별로 전자 브로셔를 표시하는 카탈로그 뷰어로서, 사용자는 eCatalog를 사용하여 추가 사용자 인터페이스 요소 또는 전용 축소판 모드를 사용하여 카탈로그를 탐색할 수 있습니다. 또한 사용자는 모든 페이지를 확대하여 자세히 볼 수 있습니다.
...
eCatalog 뷰어는 런타임에 뷰어가 다운로드한 기본 JavaScript 파일 및 도우미 파일 집합(단일 JavaScript는 이 특정 뷰어에서 사용하는 모든 뷰어 SDK 구성 요소, 자산, CSS 포함)을 나타냅니다

IS 뷰어와 함께 제공되거나 내장 모드에서 제공되는 프로덕션 준비 HTML 페이지를 사용하여 팝업 모드에서 eCatalog 뷰어를 사용할 수 있습니다. 이 HTML은 문서화된 API를 사용하여 대상 웹 페이지에 통합됩니다.
...
이 뷰어는 키보드 접근성 및 탐색에 [](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861)설명된 대로 키보드로 완벽하게 액세스할 수 있습니다.

## eCatalog 뷰어를 사용한 소셜 미디어 공유 툴 {#section-eb575084a99647c3a9591f439f40b412}

eCatalog 뷰어는 소셜 공유 도구를 지원합니다. 기본 제어 막대에서 단추를 사용하여 사용자가 클릭하거나 탭하면 공유 도구 모음으로 확장됩니다.
...
팝업 작업 모드에서는 즉시 사용 가능한 HTML 페이지를 사용하는 것이 좋습니다. 이 경우 이 매개 변수를 [!DNL eCatalogViewer.html] 호출하고 표준 IS-뷰어 배포의 [!DNL html5/] 하위 폴더 내에 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일을 추가합니다.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 포함해야 [!DNL eCatalogViewer.js]합니다. 이 [!DNL eCatalogViewer.js] 파일은 표준 IS-뷰어 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어 크기 설정
   ...
   뷰어의 정적 크기를 절대 단위로 `.s7ecatalogviewer` 최상위 CSS 클래스에 대해 선언하거나 `stagesize` 수정자를 사용하여 설정할 수 있습니다.
...
1. 뷰어를 초기화하는 중입니다.
   ...
   다음은 뷰어 인스턴스를 만들고 필요한 최소 구성 옵션을 생성자에게 전달하고 `init()` 메서드를 호출하는 예입니다. 이 예제에서는 뷰어 `eCatalogViewer` 인스턴스가 있다고 가정합니다.자리 표시자의 `s7viewer` 이름입니다. `DIV`;이미지 제공 `http://s7d1.scene7.com/is/image/` URL이며 `Viewers/Pluralist` 자산입니다.
