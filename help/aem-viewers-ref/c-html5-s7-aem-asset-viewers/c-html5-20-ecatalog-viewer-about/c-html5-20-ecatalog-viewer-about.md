...
IS-뷰어와 함께 제공되거나, 문서화된 API를 사용하여 대상 웹 페이지에 통합되는 내장 모드에서 프로덕션 준비 HTML 페이지를 사용하여 팝업 모드에서 eCatalog 뷰어를 사용할 수 있습니다.
...
eCatalog 뷰어는 다른 모바일 응용 프로그램에서 공통적인 다음 터치 제스처를 지원합니다.
...
이 뷰어는 [키보드 액세스 가능성 및 탐색](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861)에 설명된 대로 완전히 키보드에 액세스할 수 있습니다.

## eCatalog 뷰어가 포함된 소셜 미디어 공유 도구 {#section-eb575084a99647c3a9591f439f40b412}

eCatalog 뷰어는 소셜 공유 도구를 지원합니다. 기본 제어 막대의 단추로 사용할 수 있으며, 이 단추는 사용자가 클릭하거나 탭할 때 공유 도구 모음으로 확장됩니다.
...
팝업 작업 모드에 기본 HTML 페이지를 사용하는 것이 좋습니다. 이 경우 이 이름은 [!DNL eCatalogViewer.html]이며 표준 IS-Viewers 배포의 [!DNL html5/] 하위 폴더 내에 있습니다.
...
1. 웹 페이지에 뷰어 JavaScript 파일을 추가합니다.
   ...
   뷰어를 만들려면 HTML 헤드에 스크립트 태그를 추가해야 합니다. 뷰어 API를 사용하려면 먼저 [!DNL eCatalogViewer.js]을(를) 포함해야 합니다. [!DNL eCatalogViewer.js] 파일은 표준 IS-Viewers 배포의 [!DNL html5/js/] 하위 폴더 아래에 있습니다.
...
1. 뷰어를 초기화하는 중입니다.
   ...
   다음은 뷰어 인스턴스를 만들고 필요한 최소 구성 옵션을 생성자에게 전달하고 `init()` 메서드를 호출하는 예제입니다. 이 예제에서는 `eCatalogViewer`이(가) 뷰어 인스턴스라고 가정합니다.`s7viewer`은 자리 표시자 `DIV`;의 이름입니다.`http://s7d1.scene7.com/is/image/`은(는) 이미지 제공 URL이고 `Viewers/Pluralist`은(는) 자산입니다.
   ...
   다음 코드는 크기가 고정된 eCatalog 뷰어를 포함하는 간단한 웹 페이지의 완전한 예입니다.
