...
`catalog::ImageSet`에 대한 설정 응답은 `req=set` 한정자를 사용하고 네트 경로의 카탈로그 레코드 ID를 참조하여 검색할 수 있습니다. 또는 `imageset=` 수정자를 사용하여 URL에서 직접 이미지 세트를 지정할 수 있습니다. `imageset=` 한정자를 사용하여 이미지 집합을 지정하는 경우 이미지 집합 값을 이스케이프하고 포함된 한정자가 URL 쿼리 문자열의 일부로 해석되지 않도록 전체 값을 중괄호로 묶어야 합니다.
...
<table id="simpletable_3718A93699F64805A41BC8A24D7962D2"> 
 <tr class="strow"> 
  ...
  <td class="stentry"> <p><span class="codeph"> catalog::ImageSet</span>이 정의되지 않은 이미지 레코드입니다. </p></td> 
 </tr> 
 <tr class="strow"> 
...
`req=set` 요청이 수신되면 생성할 응답 유형은 `catalog::AssetType` 값으로 결정됩니다. `catalog::AssetType` 이 정의되지 않은 경우 응답 유형은 다음 규칙에 의해 결정됩니다.

* 이미지 카탈로그에 레코드가 있고 `catalog::ImageSet` 이 정의된 경우
...
외부 세트가 유형 미디어 세트로 검색되면 응답에는 `catalog::ImageSet`의 각 미디어 세트 항목에 해당하는 미디어 세트 항목 세트가 포함됩니다. 선택적 유형 매개 변수가 특정 미디어 세트 항목에 대해 지정된 경우 다음 표에 따라 출력 유형에 매핑됩니다.
...
`labelkey=` 수정자는 `catalog::UserData` 필드와 함께 사용하여 이미지 및 색상 견본의 레이블을 생성합니다. `catalog:UserData` 필드는 키/값 쌍 집합으로 구문 분석되고 지정된 키에 대한 값을 검색하도록 이 집합에 대한 레이블 키 인덱스가 구문 분석됩니다. 그런 다음 이 값은 *`s`* 및 *`i`*&#x200B;에 대한 *`l`* 특성에서 반환됩니다.

## 강제 제한 {#section-b9f042873bee45a5ae11b69fd42f2bca}

응답 크기를 제한하고 자체 참조 문제를 방지하기 위해 서버 속성 `PS::fvctx.nestingLimit`에 의해 최대 중첩 깊이가 제어됩니다. 이 제한을 초과하면 오류가 반환됩니다.

대형 전자 카탈로그 세트에 대한 xml 응답 크기를 제한하려면 서버 속성 `PS::fvctx.brochureLimit`에 따라 브로셔 세트 항목에 대한 개인 메타데이터가 표시되지 않습니다. 브로셔와 관련된 모든 비공개 메타데이터는 브로셔의 제한에 도달할 때까지 내보내집니다. 제한을 초과하면 개인 맵 및 사용자 데이터가 억제되고 해당 플래그는 숨겨진 데이터 유형을 나타내는 로 설정됩니다.
...
`req=set` 요청에 대한 샘플 XML 응답이 필요하면 HTML 예제 헤더 아래의 속성 페이지를 참조하십시오.
