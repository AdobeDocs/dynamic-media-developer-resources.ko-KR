...
## 5.15.3에 대한 새로운 개선 사항 {#new-features-and-enhancements-5-15-3}

* SDK 버전을 3.10으로 업데이트하여 레거시 모바일 HTML5 뷰어의 XSS 문제를 수정했습니다. Dynamic Media에서 사용하는 새로운 기본 HTML5 뷰어는 이 XSS 문제의 영향을 받지 않았으므로 업데이트되지 않았습니다.

## 5.15.2에 대한 새로운 개선 사항 {#new-features-and-enhancements-5-15-2}

* iOS의 기본 비디오 재생 UI 지원이 수정되었습니다.

## 5.15.1의 새로운 기능 및 개선 사항 {#new-features-and-enhancements-5-15-1}

* 수동 이벤트 리스너에 대한 지원을 추가했습니다.
* 모델 색화를 더 잘 지원하도록 DimensionalViewer에서 3D 렌더링 라이브러리를 업데이트했습니다.
* 비활성 단추에 초점을 맞추지 못했습니다.
* 공유 도구의 키보드 제어 및 보조 기술 지원이 개선되었습니다.
* iOS 장치에서 견본 스크롤 동작을 수정했습니다.
* eCatalog 뷰어에서 iOS 장치의 페이지 회전 효과를 수정했습니다.
* 수정 `swatchoverlay` 터치 장치의 견본 지원 뷰어의 보조 동작.
* 비디오가 미디어 세트에서 먼저 제공되지 않는 경우 MixedMediaViewer에서 비디오 캡션에 대한 지원이 수정되었습니다.
...
* 비디오 캡션을 사용하려면 호스팅 HTML 페이지가 외부 서버(Dynamic Media 서버가 아님)에서 제공되는 경우 회사 규칙 세트가 필요합니다. 도움이 필요하면 기술 지원 센터에 문의하십시오.
