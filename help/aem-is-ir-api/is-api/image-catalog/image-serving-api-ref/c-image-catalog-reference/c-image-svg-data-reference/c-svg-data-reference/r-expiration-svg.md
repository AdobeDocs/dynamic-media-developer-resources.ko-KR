...
특정 유형의 응답(예: 오류 응답)은 항상 즉시 만료로 표시되거나 캐시할 수 없는 것으로 태그가 지정되는 반면, 다른 유형(예: 속성 또는 기본 이미지 응답)은 특수 만료 설정( `attribute::NonImgExpiration` 및 `attribute::DefaultExpiration`).
