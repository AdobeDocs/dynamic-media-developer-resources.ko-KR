...
폴더 [!DNL Platform Server] 로그 파일을 씁니다. 이 경로는 절대 경로이거나 을 기준으로 하는 경로일 수 있습니다 *`install_folder`*. 기본값은 입니다. [!DNL  *`install_folder`*/logs].
...
액세스 로그 데이터가 기록되는 파일의 이름 접두사입니다. 날짜 및 파일 접미사( [!DNL  *`yyyy`*-*`mm`*-*`dd`*.log])가 지정된 문자열에 추가됩니다. 액세스 로그 파일의 이름은 추적 로그 파일의 이름과 달라야 합니다. 기본값은 &quot; 입니다. `access-`&quot;.

## TC::pattern - 액세스 로그 패턴 {#section-22775ea85cee444d8a7d7336a3b1feef}

에 대한 데이터 패턴을 지정합니다. [!DNL Platform Server] 로그 레코드에 액세스합니다. 패턴 문자열은 해당 값으로 대체되는 변수를 지정합니다. 패턴 문자열의 다른 모든 문자는 말 그대로 로그 레코드로 전송됩니다.

캐시 준비 유틸리티를 사용하려면 공백을 필드 구분 기호로 사용해야 합니다. 다음 [!DNL Platform Server] 필드 값의 모든 공백 및 &#39;%&#39; 문자를 `%20` 및 `%25`각각 입니다.
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>날짜 및 시간, <span class="codeph"> <span class="varname"> yyyy </span>- <span class="varname"> MM </span>- <span class="varname"> dd </span> <span class="varname"> HH </span>: <span class="varname"> mm </span>: <span class="varname"> ss </span>. <span class="varname"> SSS </span> 오프셋 </span> </p> <p> ( <span class="varname"> SSS </span> msec, <span class="varname"> 오프셋 </span> 는 GMT 시간 오프셋입니다. 시간 값은 응답이 클라이언트에 전송될 때 캡처됩니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> <p> <span class="codeph"> %R </span> </p> </td> 
   <td> <p>과 동일 <span class="codeph"> %r </span>그러나 로그 구문 분석 문제를 방지하기 위해 제한된 HTTP 인코딩을 URI에 적용합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>[!DNL Platform Server] 캐시 관리 키워드: <span class="codeph"> { 다시 사용 | 생성됨 | 업데이트됨 | 원격 | REMOTE_CREATED | REMOTE_UPDATED | REMOTE_CACHE | 유효함 | 무시됨 | 정의되지 않음 } </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>다음 <span class="codeph"> 태그 </span> 응답 헤더 값(응답 데이터의 MD5 서명). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>캐시 항목을 전달하거나 '-'인 경우 캐시 클러스터에서 피어 서버의 IP 주소입니다. <span class="codeph"> CacheUse </span> 다음 중 하나일 수 없습니다. <span class="codeph"> REMOTE_CREATED </span> 아니요 <span class="codeph"> REMOTE_UPDATED </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>의 대문자 값 <span class="codeph"> req= </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>시간이 걸리는 시간 [!DNL Platform Server] 출력 스트림에 데이터를 작성한 후 응답을 보내는 작업. </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{크기}r </span> </p> </td> 
   <td> <p>좋아요 <span class="codeph"> %B </span>, 에는 304(수정되지 않음) 응답 값이 포함되어 있습니다. </p> </td> 
  </tr> 
  <tr> 
