---
description: 검색 결과 패널은 맨 위의 검색 입력 상자와 정보 메시지 또는 검색 결과가 표시되는 주 영역으로 구성됩니다.
seo-description: 검색 결과 패널은 맨 위의 검색 입력 상자와 정보 메시지 또는 검색 결과가 표시되는 주 영역으로 구성됩니다.
seo-title: 검색 결과 패널
...
검색 결과 패널은 맨 위의 검색 입력 상자와 정보 메시지 또는 검색 결과가 표시되는 주 영역으로 구성됩니다.

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

**기본 뷰어 영역의 CSS 속성**

패널이 활성화되어 있으면 뷰어 사용자 인터페이스가 반투명 채우기로 표시됩니다. 이 채우기의 색상 및 불투명도는 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
<table id="table_94EE3F5BBE4547C0B4943471CEE7EDE4"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>색상의 불투명도 </p> </td> 
  </tr> 
 </tbody> 
</table>

검색 결과 패널은 항상 사용 가능한 모든 뷰어 높이를 차지합니다. 그러나 너비를 구성할 수 있습니다. 너비를 중간 및 큰 중단점에 대한 기본 설정인 절대 픽셀 값으로 설정할 수 있습니다. 또는 너비를 100%로 설정하여 검색 결과 패널이 전체 뷰어 영역을 차지하게 할 수 있습니다. 패널 너비는 다음과 같은 CSS 클래스 선택기에 의해 제어됩니다.
...
예 - 대형 및 중간 크기의 중단점에 250픽셀 전체 검색 결과 패널을 설정하고 작은 크기의 중단점에 전체 크기 패널을 사용하려면 다음을 수행합니다.
...
검색 결과 패널의 맨 위에는 검색 입력 상자 전용 상자가 있습니다. 입력 상자의 측면 패딩은 다음 CSS 클래스 선택기에 의해 제어됩니다.
...
<table id="table_A1B96108542742DC8DCBCC9064F9E90B"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 입력 상자 주위의 패딩. </p> </td> 
  </tr> 
 </tbody> 
</table>

검색 입력 필드는 다음 CSS 클래스 선택기로 제어됩니다.
...
예 - 0픽셀 높이 및 14픽셀 텍스트 글꼴로 검색 입력 필드를 설정하려면
...
검색 입력 필드 왼쪽에 있는 검색 버튼은 기본적으로 다음과 같은 CSS 클래스 선택기를 통해 제어됩니다.
...
<table id="table_CDD818B40BB1416CB47B7C52F799DE0C"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>"Looking glass" 아이콘 이미지의 URL. </p> </td> 
  </tr> 
  <tr> 
...
예 - 26 x 26 픽셀의 &quot;Looking glass&quot; 아이콘이 있는 검색 단추를 설정하려면30픽셀의 크기(1픽셀 테두리):
...
기능을 처음 호출할 때 검색 결과 패널에 텍스트 프롬프트가 표시될 수 있습니다. 검색 결과가 반환되지 않을 때 사용자에게 메시지가 표시됩니다. 모든 경우 텍스트가 검색 결과 패널의 주요 부분에 나타나며 다음 CSS 클래스 선택기로 제어됩니다.
...
>[!NOTE]
>
>이 텍스트 패널은 `state` 속성 선택기를 지원하므로 다른 텍스트 메시지에 다른 스타일을 적용할 수 있습니다. 특히, 패널을 처음 `state='prompt'` 호출할 때 표시되는 텍스트 프롬프트에 해당합니다.검색 히트에 대한 정보가 있는 텍스트에 `state='results'` 해당합니다.검색 쿼리가 결과를 반환하지 않을 때 표시되는 텍스트에 `state='no_results'` 해당합니다.

메시지 텍스트를 현지화할 수 있습니다. 자세한 [내용은 사용자 인터페이스 요소의](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 현지화를 참조하십시오.

예 - 회색 18픽셀 글꼴을 사용하는 텍스트 패널을 설정하려면
...
<table id="table_26974E509F6943BB98CBC1E4BAE62D68"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> margin </span> </p> </td> 
   <td colname="col2"> <p> 각 축소판의 세로 여백 크기입니다. 실제 축소판 간격은 <span class="codeph"> .s7thumbcell에 대해 설정된 위쪽 및 아래쪽 여백의 </span>합계입니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
예 - 215 x 129픽셀의 축소판을 설정하려면 밝은 회색의 기본 테두리와 어두운 회색을 선택합니다.
...
마우스 입력을 사용하는 시스템의 경우 검색 결과 패널 아래쪽에 두 개의 스크롤 단추가 나타나 사용자가 검색 결과를 스크롤합니다. 위쪽 및 아래쪽 스크롤 단추의 모양은 다음과 같은 CSS 클래스 선택기로 제어됩니다.
...
>[!NOTE]
>
>이 단추는 `state` 속성 선택기를 지원합니다. 이 `"up"`선택기는 다른 스킨을 `"down"`, `"over"``"disabled"` 및단추 상태에 적용할 수 있습니다.
...
예 - 125 x 35픽셀이고 각 상태에 대해 서로 다른 아트워크를 포함하는 스크롤 업 단추를 설정하려면:
