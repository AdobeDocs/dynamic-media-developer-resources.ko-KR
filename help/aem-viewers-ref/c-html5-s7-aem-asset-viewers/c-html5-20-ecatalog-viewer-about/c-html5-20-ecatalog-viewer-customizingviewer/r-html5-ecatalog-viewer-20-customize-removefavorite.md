...
>[!NOTE]
>
>이 단추는 `state` 및 `selected` 특성 선택기를 모두 지원합니다. 이 선택기는 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히 `selected='true'`은(는) 사용자가 클릭하거나 탭하여 새 즐겨찾기 아이콘을 추가할 수 있는 상태입니다. `selected='false'` 특성은 사용자가 페이지를 확대/축소, 패닝 및 교환할 수 있는 일반 작업 모드에 해당합니다.
