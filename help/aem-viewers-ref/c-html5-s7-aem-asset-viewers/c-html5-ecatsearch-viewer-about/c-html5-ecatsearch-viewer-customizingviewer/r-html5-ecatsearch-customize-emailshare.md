...
예 - 패딩, 24 x 17픽셀 아이콘 및 굵은 16포인트 제목을 사용하여 대화 상자 헤더를 설정하려면 다음을 수행하십시오. 그리고 마지막으로 28x28픽셀 닫기 단추가 위쪽에서 2픽셀과 대화 상자 컨테이너 오른쪽에서 2픽셀을 위치시켰습니다.
...
이 단추의 도구 설명은 현지화할 수 있습니다. 자세한 내용은 [사용자 인터페이스 요소의 로컬라이제이션](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 추가 정보.
...
예 - 모든 입력 필드 주위에 9픽셀의 패딩이 있는 1픽셀 회색 테두리를 갖도록 양식을 설정하려면 다음을 수행합니다. 유효성 검사에 실패한 필드에 대해 동일한 테두리를 빨간색 색상으로 표시하려면 다음 단계를 수행합니다. 그리고 마지막으로, 폭 250픽셀의 To 필드와 나머지 입력 필드의 너비가 300픽셀입니다.
...
사용자가 잘못된 입력 데이터를 입력하고 인라인 유효성 검사에 실패하면 대화 상자 본문의 맨 위에 메시지가 표시됩니다. 이 메시지는 양식을 제출할 때 대화 상자에서 오류 또는 확인 메시지를 렌더링해야 할 때도 나타납니다. 이 컨트롤은 다음 CSS 클래스 선택기로 제어됩니다.
...
단추 도구 팁은 현지화할 수 있습니다.

자세한 내용은 [사용자 인터페이스 요소의 로컬라이제이션](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 추가 정보.

예 - 10포인트 굵은 글꼴을 사용하도록 메시지를 설정하려면 25픽셀 라인 높이와 왼쪽에 20픽셀 패딩을 사용합니다. 마지막으로, 오류가 있으면 느낌표 아이콘, 빨간색 텍스트, 성공할 경우 아이콘과 녹색 텍스트가 없습니다.
