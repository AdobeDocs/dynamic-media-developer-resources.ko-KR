...
`ic` 는 이미지 파일을 최적화된 피라미드 TIFF 형식(PTIFF)으로 변환하는 명령줄 도구입니다. 이미지 제공은 변환 없이 이미지를 처리할 수 있지만 512x512 픽셀보다 큰 모든 이미지를 PTIFF로 변환하는 것이 좋습니다. 이 변환은 최적의 서버 성능과 리소스 사용을 보장하며 응답 시간을 최소화합니다.

사진 콘텐츠가 포함된 PTIFF 파일은 JPEG 인코딩되는 것이 좋습니다(지정) `-jpegcompress`). 컴퓨터 생성 콘텐츠는 무손실 압축(다음 중 하나)의 이점을 얻을 수 있습니다. `-deflatecompress` 또는 `-lzwcompress`). 컬러 변환 또는 픽셀 타입 변환이 필요하지 않는 한, JPEG 소스 이미지 데이터는 품질 저하를 피하기 위해 디코딩 없이 PTIFF로 전달된다. 이 경우, 지정된 압축 옵션은 낮은 해상도 피라미드 레벨에만 적용됩니다.

큰 이미지를 변환하지 않는 경우에는 사용할 메모리 양을 제어하는 매개 변수를 설정할 필요가 없습니다. 하지만, 만약 그렇다면 `ic` 를 사용하여 추가 메모리 `-maxmem` 아래에 설명되어 있는 설정입니다. 필요한 메모리의 양을 계산하는 좋은 방법은 이미지의 폭과 이미지의 높이를 곱하고 채널 수를 곱하는 것입니다. 예를 들어 알파 곱하기 3이 있는 RGB 이미지의 경우 4입니다. 또한 채널이 구성 요소당 8개가 아닌 16비트인 경우 최종 결과의 두 배가 됩니다.
...
<table id="table_E368E220299D449D8311478AB5042987"> 
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> <span class="varname"><i>dest폴더</i></span> </span> </p> </td> 
...
<table id="table_02011C7C076745A8BF4378B22C48C8A3"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>JPEG 인코딩을 사용하십시오. 다음과 같은 경우 무시됨 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 알파 데이터를 포함합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> "완벽한 표본 처리 능력" </span> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>하위 샘플링된 피라미드 레벨에 언샵 마스킹을 적용합니다. 다음을 참조하십시오 <a href="../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-command-reference/r-op-usm.md#reference-51ac75adadfe4346ab60953192d0a1aa" type="reference" format="dita" scope="local"> op_usm= </a> 을 참조하십시오. (전체 해상도 이미지에는 적용되지 않습니다.) </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>인쇄 해상도(dpi) <span class="codeph"> <span class="varname"> destFile </span> </span>; 지정되지 않은 경우 인쇄 해상도 <span class="codeph"> srcFile </span> 이(가)에 복사됩니다. <span class="codeph"> <span class="varname"> destFile </span> </span>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -autocrop &lt; <span class="varname"> 모서리 </span>&gt; &lt; <span class="varname"> 모드 </span>&gt; &lt; <span class="varname"> 허용 오차 </span>&gt; &lt; <span class="varname"> infoFile </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>자르기 사각형을 계산하여 단색 배경을 최소화합니다. 자동 자르기 알고리즘으로 인해 전체 이미지가 잘리는 경우 자르기 정보가 출력되지 않습니다. </p> <p>이미지를 변환하지 않고 자르기 사각형을 계산하려면 을 지정합니다 <span class="codeph"> -autocrop </span> 없이 <span class="codeph"> -convert </span> 및 제외 <span class="codeph"> <span class="varname"> destFile.</span> </span></p>
...
<p><i><b>모서리</b></i> - ul | url | ll | lr </p>
   ...
   <p><i><b>허용 오차</b></i> - 일치 허용치. 실수 값 0.0 ~ 1.0. 일치하는 픽셀 구성 요소 값에 대한 공차를 지정합니다. 정확히 일치하려면 0으로 설정합니다.</p>
   <p><i><b>infoFile</b></i> - 자르기 정보 데이터가 기록되는 XML 출력 파일의 경로 및 이름.</p>

<p>  
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -embedXmpData </span> </p> </td> 
   <td colname="col2"> <p>가능한 경우 다음에서 XMP 메타데이터 복사 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 끝 <span class="codeph"> <span class="varname"> destFile </span> </span> 수정 없이. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -embedColorProfile </span> </p> </td> 
   <td colname="col2"> <p> 에 ICC 색상 프로파일 포함 <span class="codeph"> <span class="varname"> destFile </span> </span>, 사용 가능한 경우(기본적으로 임베드된 프로필 없음). </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -imageprofile &lt; <span class="varname"> 파일 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>ICC 프로파일 파일의 경로 및 이름입니다. 의 색상 공간 정의 <span class="codeph"> <span class="varname"> sourceFile </span> </span> 및 은(는) 픽셀 유형과 일치해야 합니다. 에 임베드된 프로필이 없는 경우에만 지정해야 함 <span class="codeph"> <span class="varname"> sourceFile </span> </span>: 임베드된 프로필을 무시합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -viewprofile &lt; <span class="varname"> 파일 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>ICC 프로파일 파일의 경로 및 이름입니다. 의 픽셀 유형 및 색상 공간을 정의합니다. <span class="codeph"> <span class="varname"> destFile </span> </span>. 다음과 같은 경우 IC가 이 프로필로 전환됩니다. <span class="codeph"> <span class="varname"> sourceFile </span> </span> 에 임베드된 프로파일이 있거나 <span class="codeph"> -imageprofile </span> 도 지정됩니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>특정 색상 변환에 대한 검은 점 보상 비활성화 </p> <p>기본적으로 활성화됨. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -누세 </span> </p> </td> 
   <td colname="col2"> <p> 첫 번째 옵션으로 지정한 경우 잘못된 옵션이 발견되면 사용 정보의 출력을 억제합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -덮어쓰기 </span> </p> </td> 
   <td colname="col2"> <p>기존 덮어쓰기 허용 <span class="codeph"> <span class="varname"> destFile </span> </span>. 기본적으로 덮어쓰기를 방지하기 위해 파일 이름에 숫자 접미사가 추가됩니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -maxmempercent &lt; <span class="varname"> 백분율 </span>&gt; </span> </p> </td> 
   <td colname="col2"> <p>메모리 사용 제한. 기본값은 물리적 메모리의 25%입니다. 둘 다 아닌 경우 <span class="codeph"> maxmem </span> nor <span class="codeph"> maxmempercent </span> 명시적으로 설정된 경우 maxmempercent 기본값을 사용합니다. </p> </td> 
  </tr> 
  <tr> 
...
모든 이미지 변환 *`srcFolder`* JPEG으로 인코딩된 피라미드 TIFF 및 배치 *`destFolder`*:
