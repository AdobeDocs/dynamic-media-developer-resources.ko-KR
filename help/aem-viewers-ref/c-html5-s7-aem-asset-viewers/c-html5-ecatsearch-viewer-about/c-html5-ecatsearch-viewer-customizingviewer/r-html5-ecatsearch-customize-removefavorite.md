...
>[!NOTE]
>
>이 버튼은 `state` 및 `selected` 속성 선택기 : 다른 단추 상태에 다른 스킨을 적용하는 데 사용할 수 있습니다. 특히, `selected='true'` 는 사용자가 클릭하거나 탭하여 새 즐겨찾기 아이콘을 추가할 수 있는 상태에 해당합니다. 및 `selected='false'` 사용자가 페이지를 확대/축소, 패닝 및 스왑할 수 있는 경우 일반 작업 모드에 해당합니다.
