...
* 라인 수는 의 최대값을 초과하지 않습니다. `\copyfitmaxlines` 텍스트 입력 시 명시적인 줄 수 및 설정.
* 이미지 집합에 일치하는 중괄호와 괄호가 필요합니다. 중괄호와 괄호가 일치하지 않으면 URL로 인코딩해야 합니다.
* 서버측 글로벌 응답 시간 경고에 오류 응답이 포함됩니다.
* 다음 `id=` 을(를) 사용할 때 현재 명령이 필요합니다. `rect=` 이미지 또는 마스크 요청이 있는 명령입니다.

## 알려진 차이점 textPs= 와 text= {#section-16ede4c13a7648feb0d2fc93341fd4aa}

* 합성 기울임꼴 은 를 사용할 때보다 기울기가 약해집니다 `text=`.
* 밑줄은 사용할 때보다 약간 더 낮고 얇습니다 `text=`.
* `\expnd` 및 `\expndtw` 음수 값이 높은 상태로 사용하면 를 사용할 때 문자가 서로 앞에 놓입니다. `text=`.

* `\charscaley` 를 사용할 때와 다르게 크기 조절 `text=` 하지만 선 높이에는 영향을 주지 않습니다.

* 텍스트의 마지막 줄이 맞지 않으면 잘라내기로 표시되지 않고 전체 줄이 삭제됩니다.
* `\slmult` 및 `\sl` MS Word와 다르게 동작하며 `text=`, 현재 및 후속 단락에 대해서만 적용됩니다.

* `\sb` MS Word 및 의 첫 번째 단락에 적용됩니다. `text=`, Adobe InDesign 및 [!DNL Photoshop] 이러지 마

* `\sa` MS Word 및 의 마지막 단락에 모두 적용됩니다. `text=`, Adobe InDesign 및 [!DNL Photoshop] 이러지 마

## 이전 버전과의 호환성 {#section-a76842f751944f4fb664af296d064122}

* 밑줄 문자 이스케이프 처리( `\_`) RTF 문자열의 일부 글꼴에서는 `textPs=`
...
* `posN=`, `anchor=`, `anchorN=`, `origin=`, 및 `originN=` 이제 수정자 값 중 하나라도 2147483648보다 큰 경우 구문 분석 오류를 반환합니다.

* 중첩 요청의 인코딩은 지원되지 않습니다. 새 동작으로 전환하고 사이트의 URL 요청과 회사 카탈로그에 있는 중첩된 요청 값의 인코딩을 해제합니다.
* 이제 사용할 때 DefaultImage가 썸네일 속성을 적용합니다. `req=tmb`.
* 이전 버전에서는 `flip=`에서는 기준점이 무엇인지에 관계없이 이미지의 위치가 변경되지 않았습니다.
...
* 설치 제거에서 사용자가 만들었거나 수정한 파일이 제거될 수 있습니다. *[!DNL install_root]* 또는 모든 하위 폴더 제거하기 전에 이러한 파일을 다른 위치에 복사합니다.

## 이미지 제공에만 적용할 수 있는 제한 사항 {#section-b08ad535e4454265b8157dec244c4faf}

* RTF의 전경색( `\cf`) 명령이 PhotoFont 텍스트에 지원되지 않습니다.
...
* 회사에서 로케일 번역을 사용하도록 설정한 경우 파일 기반 조회가 지원되지 않음 [!DNL ini] 파일.
* 이미지 제공이 닫히지 않은 상태로 기록되지 않음 [!DNL Photoshop] 경로가 올바르게 표시됩니다.
...
* 다음의 경우 색상 변환이 올바르지 않을 수 있습니다. `bgColor=` 는 색상 프로파일을 포함하는 기본 색상 공간과 일치하지 않습니다.
...
`ImageConvert`로 중지되면 때로 세그멘테이션 오류와 충돌합니다. `control-c`.
