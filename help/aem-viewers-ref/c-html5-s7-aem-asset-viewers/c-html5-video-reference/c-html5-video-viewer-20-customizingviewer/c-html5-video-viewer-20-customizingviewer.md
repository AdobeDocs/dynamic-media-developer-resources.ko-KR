...
사용자 지정 CSS를 만들 때는 뷰어가 `.s7videoviewer` 클래스를 컨테이너 DOM 요소에 추가합니다. 와 함께 전달되는 외부 CSS 파일을 사용하는 경우 `style=` 명령, 사용 `.s7videoviewer` 클래스를 CSS 규칙에 대한 하위 항목 선택기의 상위 클래스로 사용합니다. 웹 페이지에 스타일을 포함하는 경우 다음과 같이 이 선택기에 컨테이너 DOM 요소의 ID도 부여합니다.
